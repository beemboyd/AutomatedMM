...
def setup_logging():
    ...
    # Create place_orders_hourly.log in the log directory
    log_file = os.path.join(log_dir, 'place_orders_hourly.log')
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Place hourly MIS orders based on BTPB signals")
    parser.add_argument(
        "-f", "--signal-file", 
...
def place_orders_from_btpb(order_manager, signal_file, max_positions=5):
    ...
    try:
        ...
        for ticker, info in order_info.items():
            ...
            qty = info["quantity"]
            # Ensure MIS product type for hourly orders
            ...
            if success:
                ...
                logger.info(f"Successfully placed MIS BUY order for {ticker} with quantity {qty}")
            else:
                logger.error(f"Failed to place MIS BUY order for {ticker}")
        ...
        if orders_placed:
            ...
            for ticker, order_data in orders_placed.items():
                ...
                # Ensure MIS product type for GTT orders
                original_product_type = order_manager.product_type
                order_manager.product_type = "MIS"
...
def main():
    ...
    logger.info(f"===== Hourly Order Placement Started at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} =====")
    
    # Get configuration
    config = get_config()
    
    # Verify that the product type is set to MIS
    product_type = config.get('Trading', 'product_type')
    if product_type != "MIS":
        logger.warning(f"Config product_type is {product_type}, but this script is designed for MIS orders.")
        logger.warning("Will override to use MIS for orders, but consider updating your config if this is unintended.")
    ...
    if not signal_file:
        logger.info("Signal file not specified, looking for latest BTPB_Signals_hourly file...")
        data_dir = config.get('System', 'data_dir')
        
        # Find the latest BTPB_Signals_hourly file
        files = [f for f in os.listdir(data_dir) if f.startswith('BTPB_Signals_hourly_') and (f.endswith('.xlsx') or f.endswith('.txt'))]
        if files:
            ...
            logger.error("No BTPB_Signals_hourly file found in data directory")
            return
    ...
    # Place orders
    try:
        logger.info("=== Processing BTPB signals for hourly MIS orders ===")
        place_orders_from_btpb(order_manager, signal_file, max_positions)
    except Exception as e:
        logger.exception(f"Error during order placement: {e}")
    
    # Log end of execution
    logger.info(f"===== Hourly Order Placement Completed at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} =====\n")
