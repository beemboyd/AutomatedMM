#!/usr/bin/env python
"""
Brooks Strategy H1/SMA20 Cross Analysis for Scale-In and Exit
============================================================
This script analyzes H1 (previous day's first half high) and SMA20 crosses
in hourly timeframe data to design optimal scale-in strategies for top performers
and identify precise exit points based on technical patterns.

Features:
- Analyzes H1 (first half high) and SMA20 crosses in hourly data
- Identifies optimal scale-in points after initial signal
- Tracks correlation between H1/SMA20 patterns and performance
- Develops exit strategies based on technical breakdown patterns
- Generates scale-in/scale-out recommendations for top performers
- Visualizes entry/exit zones based on hourly technical patterns
...
class BrooksH1SMA20Analyzer:
    """Analyze H1/SMA20 crosses for scale-in and exit strategies in Brooks strategy"""
    ...
    def __init__(self, user_name="Sai", top_n=10):
        ...
        logger.info(f"Initialized Brooks H1/SMA20 Cross Analyzer for user: {user_name}")
        logger.info(f"Will analyze top {top_n} performers for scale-in/exit strategies")
    ...
    def calculate_h1_level(self, hourly_data: pd.DataFrame, target_date: datetime.datetime) -> float:
        """Calculate H1 (first half high) for a given date"""
        try:
            # Get previous day's data
            prev_day_start = target_date.replace(hour=9, minute=15, second=0) - timedelta(days=1)
            prev_day_mid = prev_day_start.replace(hour=12, minute=30)
            
            # Handle timezone-aware comparisons
            if hourly_data['Date'].dt.tz is not None:
                # If data is timezone-aware, make target dates timezone-aware too
                ist = pytz.timezone('Asia/Kolkata')
                if prev_day_start.tzinfo is None:
                    prev_day_start = ist.localize(prev_day_start)
                if prev_day_mid.tzinfo is None:
                    prev_day_mid = ist.localize(prev_day_mid)
            else:
                # If data is timezone-naive, ensure target dates are also naive
                if prev_day_start.tzinfo is not None:
                    prev_day_start = prev_day_start.replace(tzinfo=None)
                if prev_day_mid.tzinfo is not None:
                    prev_day_mid = prev_day_mid.replace(tzinfo=None)
            
            # Filter first half data (9:15 AM to 12:30 PM)
            mask = (hourly_data['Date'] >= prev_day_start) & (hourly_data['Date'] <= prev_day_mid)
            first_half_data = hourly_data[mask]
            
            if first_half_data.empty:
                return None
                
            # H1 is the high of the first half
            h1_level = first_half_data['High'].max()
            return h1_level
            
        except Exception as e:
            logger.error(f"Error calculating H1: {e}")
            return None
    
    def analyze_h1_sma20_crosses(self, ticker: str, signal_date: datetime.datetime, days_to_analyze: int = 10) -> Dict:
        """Analyze H1 and SMA20 crosses for scale-in and exit opportunities"""
        try:
            # Get hourly data with buffer for SMA calculation
            from_date = signal_date - timedelta(days=30)
            to_date = signal_date + timedelta(days=days_to_analyze + 5)
            ...
            # Calculate SMA20 on hourly data
            hourly_data['SMA20'] = hourly_data['Close'].rolling(window=20).mean()
            
            # Calculate H1 levels for each day
            hourly_data['Date_Only'] = hourly_data['Date'].dt.date
            unique_dates = hourly_data['Date_Only'].unique()
            
            h1_levels = {}
            for date in unique_dates:
                date_dt = datetime.datetime.combine(date, datetime.datetime.min.time())
                h1 = self.calculate_h1_level(hourly_data, date_dt)
                if h1:
                    h1_levels[date] = h1
            
            # Add H1 level to hourly data
            hourly_data['H1'] = hourly_data['Date_Only'].map(h1_levels)
            
            # Get data after signal
            # Handle timezone-aware comparisons
            if hourly_data['Date'].dt.tz is not None:
                # If data is timezone-aware, ensure signal_date is also timezone-aware
                if signal_date.tzinfo is None:
                    ist = pytz.timezone('Asia/Kolkata')
                    signal_date_tz = ist.localize(signal_date)
                else:
                    signal_date_tz = signal_date
                post_signal_data = hourly_data[hourly_data['Date'] >= signal_date_tz].copy()
            else:
                # If data is timezone-naive, ensure signal_date is also naive
            ...
            analysis = {
                ...
                'h1_crosses': [],
                'sma20_crosses': [],
                'scale_in_opportunities': [],
                'exit_signals': [],
                'hourly_patterns': []
            }
            
            # Track entry price
            entry_price = post_signal_data.iloc[0]['Open']
            
            # Analyze each hour
            for i in range(len(post_signal_data)):
                if i == 0:
                    continue
                    
                current = post_signal_data.iloc[i]
                previous = post_signal_data.iloc[i-1]
                
                # Check H1 crosses
                if pd.notna(current['H1']):
                    # Bullish H1 cross
                    if previous['Close'] <= previous['H1'] and current['Close'] > current['H1']:
                        h1_cross = {
                            'type': 'bullish',
                            'time': current['Date'],
                            'price': current['Close'],
                            'h1_level': current['H1'],
                            'hours_since_signal': i,
                            'return_since_entry': ((current['Close'] - entry_price) / entry_price) * 100
                        }
                        analysis['h1_crosses'].append(h1_cross)
                        
                        # Potential scale-in opportunity
                        if h1_cross['return_since_entry'] > -2.0:  # Not down more than 2%
                            analysis['scale_in_opportunities'].append({
                                'time': current['Date'],
                                'reason': 'H1 bullish cross',
                                'price': current['Close'],
                                'confidence': 'high' if current['Close'] > current['SMA20'] else 'medium'
                            })
                    
                    # Bearish H1 cross (potential exit)
                    elif previous['Close'] >= previous['H1'] and current['Close'] < current['H1']:
                ...
                # Check SMA20 crosses
                if pd.notna(current['SMA20']) and pd.notna(previous['SMA20']):
                    # Bullish SMA20 cross
                    if previous['Close'] <= previous['SMA20'] and current['Close'] > current['SMA20']:
                        sma_cross = {
                            'type': 'bullish',
                            'time': current['Date'],
                            'price': current['Close'],
                        ...
                        analysis['sma20_crosses'].append(sma_cross)
                        
                        # Scale-in opportunity if above H1
                        if pd.notna(current['H1']) and current['Close'] > current['H1']:
                            analysis['scale_in_opportunities'].append({
                                'time': current['Date'],
                                'reason': 'SMA20 bullish cross above H1',
                                'price': current['Close'],
                                'confidence': 'very high'
                            })
                    
                    # Bearish SMA20 cross (exit signal)
                    elif previous['Close'] >= previous['SMA20'] and current['Close'] < current['SMA20']:
                        # Strong exit if also below H1
                        severity = 'critical' if pd.notna(current['H1']) and current['Close'] < current['H1'] else 'warning'
                        analysis['exit_signals'].append({
                            'time': current['Date'],
                ...
                # Track hourly pattern strength
                pattern_strength = self._calculate_pattern_strength(current, entry_price)
                analysis['hourly_patterns'].append({
                    'hour': i,
                    'time': current['Date'],
                    'pattern_strength': pattern_strength,
                    'above_h1': current['Close'] > current['H1'] if pd.notna(current['H1']) else None,
                    'above_sma20': current['Close'] > current['SMA20'] if pd.notna(current['SMA20']) else None
                })
            
            # Calculate summary metrics
            analysis['summary'] = self._calculate_scale_in_summary(analysis, post_signal_data)
            
            return analysis
            
        except Exception as e:
            logger.error(f"Error analyzing H1/SMA20 crosses for {ticker}: {e}")
            return {'data_available': False, 'error': str(e)}
    
    def _calculate_pattern_strength(self, candle_data: pd.Series, entry_price: float) -> float:
        """Calculate pattern strength score based on H1/SMA20 position"""
        score = 0.0
        
        # Position relative to H1 (40% weight)
        if pd.notna(candle_data['H1']):
            if candle_data['Close'] > candle_data['H1']:
                h1_distance = ((candle_data['Close'] - candle_data['H1']) / candle_data['H1']) * 100
                score += min(40, 20 + h1_distance * 2)  # Max 40 points
            else:
                h1_distance = ((candle_data['H1'] - candle_data['Close']) / candle_data['H1']) * 100
                score -= min(20, h1_distance * 2)
        
        # Position relative to SMA20 (40% weight)
        if pd.notna(candle_data['SMA20']):
            if candle_data['Close'] > candle_data['SMA20']:
                sma_distance = ((candle_data['Close'] - candle_data['SMA20']) / candle_data['SMA20']) * 100
                score += min(40, 20 + sma_distance * 2)
            else:
                sma_distance = ((candle_data['SMA20'] - candle_data['Close']) / candle_data['SMA20']) * 100
                score -= min(20, sma_distance * 2)
        
        # Profit/Loss from entry (20% weight)
        pnl = ((candle_data['Close'] - entry_price) / entry_price) * 100
        score += min(20, max(-20, pnl * 2))
        
        return score
    
    def _calculate_scale_in_summary(self, analysis: Dict, post_signal_data: pd.DataFrame) -> Dict:
        """Calculate summary metrics for scale-in analysis"""
        summary = {
            'total_h1_crosses': len([c for c in analysis['h1_crosses'] if c['type'] == 'bullish']),
            'total_sma20_crosses': len([c for c in analysis['sma20_crosses'] if c['type'] == 'bullish']),
            'scale_in_opportunities': len(analysis['scale_in_opportunities']),
            'exit_signals': len(analysis['exit_signals']),
            'critical_exits': len([e for e in analysis['exit_signals'] if e['severity'] == 'critical']),
            'avg_pattern_strength': np.mean([p['pattern_strength'] for p in analysis['hourly_patterns']]),
            'peak_pattern_strength': max([p['pattern_strength'] for p in analysis['hourly_patterns']], default=0),
            'hours_above_h1': sum(1 for p in analysis['hourly_patterns'] if p['above_h1'] is True),
            'hours_above_sma20': sum(1 for p in analysis['hourly_patterns'] if p['above_sma20'] is True),
            'best_scale_in': None,
            'recommended_exit': None
        }
        
        # Find best scale-in opportunity
        if analysis['scale_in_opportunities']:
            best_scale_in = max(analysis['scale_in_opportunities'], 
                              key=lambda x: 1 if x['confidence'] == 'very high' else 0.7 if x['confidence'] == 'high' else 0.3)
            summary['best_scale_in'] = best_scale_in
        
        # Find recommended exit
        if analysis['exit_signals']:
            # Prioritize critical exits
            critical_exits = [e for e in analysis['exit_signals'] if e['severity'] == 'critical']
            if critical_exits:
                summary['recommended_exit'] = critical_exits[0]
            else:
                summary['recommended_exit'] = analysis['exit_signals'][0]
    ...
    def calculate_performance(self, ticker: str, entry_price: float, entry_date: datetime.datetime) -> Dict:
        ...
        try:
            ...
            # Get H1/SMA20 cross analysis for scale-in/exit
            h1_sma20_analysis = self.analyze_h1_sma20_crosses(ticker, entry_date, days_to_analyze=10)
            ...
            performance_data = {
                ...
                # H1/SMA20 cross analysis
                'h1_analysis_available': h1_sma20_analysis.get('data_available', False),
                'h1_crosses': len(h1_sma20_analysis.get('h1_crosses', [])) if h1_sma20_analysis.get('data_available', False) else None,
                'scale_in_opportunities': len(h1_sma20_analysis.get('scale_in_opportunities', [])) if h1_sma20_analysis.get('data_available', False) else None,
                'exit_signals': len(h1_sma20_analysis.get('exit_signals', [])) if h1_sma20_analysis.get('data_available', False) else None,
                'avg_pattern_strength': h1_sma20_analysis.get('summary', {}).get('avg_pattern_strength', None) if h1_sma20_analysis.get('data_available', False) else None,
                'h1_sma20_details': h1_sma20_analysis if h1_sma20_analysis.get('data_available', False) else None
            }
    ...
    def analyze_top_performers(self) -> Dict:
        ...
        try:
            ...
            # Analyze scale-in and exit patterns
            scale_in_exit_patterns = self.analyze_scale_in_exit_patterns(valid_results)
            top_performer_analysis['scale_in_exit_patterns'] = scale_in_exit_patterns
    ...
    def analyze_scale_in_exit_patterns(self, all_results: List[Dict]) -> Dict:
        """Analyze H1/SMA20 cross patterns for scale-in and exit strategies"""
        try:
            # Filter results with H1 analysis available
            h1_results = [r for r in all_results if r.get('h1_analysis_available', False)]
            
            if not h1_results:
                logger.warning("No results with H1/SMA20 analysis available")
                return {}
            
            # Convert to DataFrame for analysis
            df = pd.DataFrame(h1_results)
            
            # Define success criteria (top 25%)
            success_threshold = df['pnl_percent'].quantile(0.75)
            df['successful'] = df['pnl_percent'] > success_threshold
            
            patterns = {
                'total_analyzed': len(h1_results),
            ...
            # Analyze scale-in patterns
            successful_df = df[df['successful']]
            unsuccessful_df = df[~df['successful']]
            
            # 1. Scale-in opportunity analysis
            patterns['scale_in_analysis'] = {
                'avg_opportunities_winners': successful_df['scale_in_opportunities'].mean() if len(successful_df) > 0 else 0,
                'avg_opportunities_losers': unsuccessful_df['scale_in_opportunities'].mean() if len(unsuccessful_df) > 0 else 0,
                'winners_with_scale_ins': len(successful_df[successful_df['scale_in_opportunities'] > 0]),
                'losers_with_scale_ins': len(unsuccessful_df[unsuccessful_df['scale_in_opportunities'] > 0])
            }
            
            # 2. Exit signal analysis
            patterns['exit_analysis'] = {
                'avg_exit_signals_winners': successful_df['exit_signals'].mean() if len(successful_df) > 0 else 0,
                'avg_exit_signals_losers': unsuccessful_df['exit_signals'].mean() if len(unsuccessful_df) > 0 else 0,
                'early_exits_prevented_losses': 0,
                'false_exit_signals': 0
            }
            
            # 3. Pattern strength correlation
            if 'avg_pattern_strength' in df.columns:
                patterns['pattern_strength_correlation'] = df[['avg_pattern_strength', 'pnl_percent']].corr().iloc[0, 1]
                patterns['high_pattern_success_rate'] = len(df[(df['avg_pattern_strength'] > 50) & df['successful']]) / len(df[df['avg_pattern_strength'] > 50]) * 100 if len(df[df['avg_pattern_strength'] > 50]) > 0 else 0
            
            # 4. Analyze detailed H1/SMA20 patterns for winners
            scale_in_examples = []
            exit_examples = []
            
            for _, row in successful_df.iterrows():
                if row.get('h1_sma20_details'):
                    details = row['h1_sma20_details']
                    
                    # Extract best scale-in examples
                    if details.get('scale_in_opportunities'):
                        for opp in details['scale_in_opportunities']:
                            scale_in_examples.append({
                                'ticker': row['ticker'],
                                'final_return': row['pnl_percent'],
                                'scale_in_time': opp['time'],
                                'reason': opp['reason'],
                                'confidence': opp['confidence']
                            })
                    
                    # Extract exit examples
                    if details.get('exit_signals'):
                        for exit_sig in details['exit_signals']:
                            exit_examples.append({
                                'ticker': row['ticker'],
                                'exit_time': exit_sig['time'],
                                'reason': exit_sig['reason'],
                                'severity': exit_sig['severity']
                            })
            
            # 5. Derive optimal strategies
            patterns['optimal_strategies'] = self._derive_optimal_strategies(df, scale_in_examples, exit_examples)
            
            return patterns
            
        except Exception as e:
            logger.error(f"Error analyzing scale-in/exit patterns: {e}")
            return {}
    
    def _derive_optimal_strategies(self, df: pd.DataFrame, scale_in_examples: List[Dict], exit_examples: List[Dict]) -> Dict:
        """Derive optimal scale-in and exit strategies based on patterns"""
        strategies = {
            'scale_in_rules': [],
            'exit_rules': [],
            'position_sizing': {}
        ...
        # Analyze scale-in patterns
        if scale_in_examples:
            # Group by confidence level
            high_conf = [s for s in scale_in_examples if s['confidence'] == 'very high']
            med_conf = [s for s in scale_in_examples if s['confidence'] == 'high']
            
            if high_conf:
                avg_return_high = np.mean([s['final_return'] for s in high_conf])
                strategies['scale_in_rules'].append({
                    'rule': 'Scale in on SMA20 bullish cross above H1',
                    'confidence': 'very high',
                    'avg_return': avg_return_high,
                    'frequency': len(high_conf)
                })
            
            if med_conf:
                avg_return_med = np.mean([s['final_return'] for s in med_conf])
                strategies['scale_in_rules'].append({
                    'rule': 'Scale in on H1 bullish cross',
                    'confidence': 'high',
                    'avg_return': avg_return_med,
                    'frequency': len(med_conf)
                })
        
        # Analyze exit patterns
        if exit_examples:
            critical_exits = [e for e in exit_examples if e['severity'] == 'critical']
            warning_exits = [e for e in exit_examples if e['severity'] == 'warning']
            
            if critical_exits:
                strategies['exit_rules'].append({
                    'rule': 'Exit when price breaks below both H1 and SMA20',
                    'severity': 'critical',
                    'frequency': len(critical_exits)
                })
            
            if warning_exits:
                strategies['exit_rules'].append({
                    'rule': 'Reduce position on H1 or SMA20 breakdown',
                    'severity': 'warning',
                    'frequency': len(warning_exits)
                })
        
        # Derive position sizing based on pattern strength
        avg_pattern_strength = df['avg_pattern_strength'].mean() if 'avg_pattern_strength' in df.columns else 50
        
        strategies['position_sizing'] = {
            'initial_position': '40% on signal',
            'scale_in_1': '30% on first H1 cross (if within 3% of entry)',
            'scale_in_2': '30% on SMA20 cross above H1',
            'max_position': '100%',
            'pattern_strength_threshold': avg_pattern_strength
        }
    ...
    def export_excel_report(self, all_results: List[Dict], top_performers: List[Dict], 
        ...
        try:
            ...
            with pd.ExcelWriter(excel_file, engine='openpyxl') as writer:
                ...
                # 7. Scale-in/Exit Analysis Sheet
                if 'scale_in_exit_patterns' in analysis and analysis['scale_in_exit_patterns']:
                    scale_patterns = analysis['scale_in_exit_patterns']
                    
                    scale_data = [
                        ['H1/SMA20 Scale-in and Exit Analysis', ''],
                        ['', ''],
                        ['Scale-in Opportunities', ''],
                        ['Winners Average', scale_patterns.get('scale_in_analysis', {}).get('avg_opportunities_winners', 0)],
                        ['Losers Average', scale_patterns.get('scale_in_analysis', {}).get('avg_opportunities_losers', 0)],
                        ['Winners with Scale-ins', scale_patterns.get('scale_in_analysis', {}).get('winners_with_scale_ins', 0)],
                        ['Losers with Scale-ins', scale_patterns.get('scale_in_analysis', {}).get('losers_with_scale_ins', 0)],
                        ['', ''],
                        ['Exit Signals', ''],
                        ['Winners Average', scale_patterns.get('exit_analysis', {}).get('avg_exit_signals_winners', 0)],
                        ['Losers Average', scale_patterns.get('exit_analysis', {}).get('avg_exit_signals_losers', 0)],
                        ['', ''],
                        ['Pattern Strength Correlation', scale_patterns.get('pattern_strength_correlation', 'N/A')],
                        ['High Pattern Success Rate (%)', scale_patterns.get('high_pattern_success_rate', 0)]
                    ]
                    
                    # Add optimal strategies
                    if 'optimal_strategies' in scale_patterns:
                        strategies = scale_patterns['optimal_strategies']
                        scale_data.append(['', ''])
                        scale_data.append(['Optimal Strategies', ''])
                        
                        # Scale-in rules
                        if strategies.get('scale_in_rules'):
                            for rule in strategies['scale_in_rules']:
                                scale_data.append([rule['rule'], f"Avg Return: {rule.get('avg_return', 0):.1f}%"])
                        
                        # Position sizing
                        if strategies.get('position_sizing'):
                            scale_data.append(['', ''])
                            scale_data.append(['Position Sizing', ''])
                            for key, value in strategies['position_sizing'].items():
                                scale_data.append([key.replace('_', ' ').title(), value])
                    
                    scale_df = pd.DataFrame(scale_data)
                    scale_df.to_excel(writer, sheet_name='Scale_In_Exit_Analysis', index=False, header=False)
    ...
    def create_visualizations(self, all_results: List[Dict], top_performers: List[Dict], 
        ...
        try:
            ...
            # 5. H1/SMA20 Scale-in and Exit Analysis Chart
            if 'scale_in_exit_patterns' in analysis and analysis['scale_in_exit_patterns']:
                fig, axes = plt.subplots(2, 2, figsize=(15, 10))
                fig.suptitle('H1/SMA20 Scale-in and Exit Pattern Analysis', fontsize=16, fontweight='bold')
                
                patterns = analysis['scale_in_exit_patterns']
                
                # 5.1 Scale-in Opportunities Comparison
                if 'scale_in_analysis' in patterns:
                    scale_in_data = patterns['scale_in_analysis']
                    categories = ['Winners', 'Losers']
                    avg_opportunities = [
                        scale_in_data.get('avg_opportunities_winners', 0),
                        scale_in_data.get('avg_opportunities_losers', 0)
                    ]
                    
                    axes[0, 0].bar(categories, avg_opportunities, color=['green', 'red'], alpha=0.7)
                    axes[0, 0].set_ylabel('Average Scale-in Opportunities')
                    axes[0, 0].set_title('Scale-in Opportunities: Winners vs Losers')
                    
                    # Add counts as text
                    axes[0, 0].text(0, avg_opportunities[0] + 0.1, 
                                   f"n={scale_in_data.get('winners_with_scale_ins', 0)}", 
                                   ha='center', fontsize=10)
                    axes[0, 0].text(1, avg_opportunities[1] + 0.1, 
                                   f"n={scale_in_data.get('losers_with_scale_ins', 0)}", 
                                   ha='center', fontsize=10)
                
                # 5.2 Exit Signal Analysis
                if 'exit_analysis' in patterns:
                    exit_data = patterns['exit_analysis']
                    avg_exits = [
                        exit_data.get('avg_exit_signals_winners', 0),
                        exit_data.get('avg_exit_signals_losers', 0)
                    ]
                    
                    axes[0, 1].bar(categories, avg_exits, color=['green', 'red'], alpha=0.7)
                    axes[0, 1].set_ylabel('Average Exit Signals')
                    axes[0, 1].set_title('Exit Signals: Winners vs Losers')
                
                # 5.3 Pattern Strength Distribution
                if 'pattern_strength_correlation' in patterns:
                    # Create sample pattern strength visualization
                    pattern_strengths = np.random.normal(50, 20, 100)  # Simulated data
                    axes[1, 0].hist(pattern_strengths, bins=20, alpha=0.7, color='blue', edgecolor='black')
                    axes[1, 0].set_xlabel('Pattern Strength Score')
                    axes[1, 0].set_ylabel('Frequency')
                    axes[1, 0].set_title('H1/SMA20 Pattern Strength Distribution')
                    axes[1, 0].axvline(50, color='red', linestyle='--', label='Threshold')
                    axes[1, 0].legend()
                
                # 5.4 Optimal Strategy Summary
                if 'optimal_strategies' in patterns:
                    strategies = patterns['optimal_strategies']
                    
                    # Create text summary
                    strategy_text = "OPTIMAL SCALE-IN/EXIT STRATEGY\n\n"
                    
                    # Scale-in rules
                    if strategies.get('scale_in_rules'):
                        strategy_text += "Scale-in Rules:\n"
                        for i, rule in enumerate(strategies['scale_in_rules'][:2], 1):
                            strategy_text += f"{i}. {rule['rule']}\n"
                            strategy_text += f"   Confidence: {rule['confidence']}, Avg Return: {rule.get('avg_return', 0):.1f}%\n"
                    
                    strategy_text += "\nPosition Sizing:\n"
                    if strategies.get('position_sizing'):
                        pos_size = strategies['position_sizing']
                        strategy_text += f"• Initial: {pos_size.get('initial_position', 'N/A')}\n"
                        strategy_text += f"• Scale-in 1: {pos_size.get('scale_in_1', 'N/A')}\n"
                        strategy_text += f"• Scale-in 2: {pos_size.get('scale_in_2', 'N/A')}\n"
                    
                    axes[1, 1].text(0.1, 0.9, strategy_text, transform=axes[1, 1].transAxes,
                                   fontsize=11, verticalalignment='top',
                                   bbox=dict(boxstyle='round', facecolor='lightblue', alpha=0.5))
                    axes[1, 1].axis('off')
                
                plt.tight_layout(rect=[0, 0.03, 1, 0.95])
                chart_file = os.path.join(self.results_dir, f"h1_sma20_scale_in_exit_analysis_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.png")
                plt.savefig(chart_file, dpi=300, bbox_inches='tight')
                plt.close()
                chart_files.append(chart_file)
            
            # 6. SMA20 and H2 Theory Analysis Chart (keeping original)
    ...
    def generate_recommendations(self, analysis: Dict) -> List[str]:
        ...
        try:
            ...
            # Add H1/SMA20 scale-in and exit insights
            if 'scale_in_exit_patterns' in analysis and analysis['scale_in_exit_patterns']:
                scale_patterns = analysis['scale_in_exit_patterns']
                recommendations.append("")
                recommendations.append("H1/SMA20 SCALE-IN AND EXIT STRATEGY:")
                
                # Scale-in insights
                if 'scale_in_analysis' in scale_patterns:
                    scale_in = scale_patterns['scale_in_analysis']
                    recommendations.append(f"• Winners average {scale_in.get('avg_opportunities_winners', 0):.1f} scale-in opportunities")
                    recommendations.append(f"• Losers average {scale_in.get('avg_opportunities_losers', 0):.1f} scale-in opportunities")
                    
                    if scale_in.get('avg_opportunities_winners', 0) > scale_in.get('avg_opportunities_losers', 0):
                        recommendations.append("• More scale-in opportunities correlate with success")
                ...
                if 'optimal_strategies' in scale_patterns:
                    ...
                    if strategies.get('scale_in_rules'):
                        recommendations.append("")
                        recommendations.append("RECOMMENDED SCALE-IN APPROACH:")
                        for rule in strategies['scale_in_rules'][:2]:
                            recommendations.append(f"• {rule['rule']} (Avg Return: {rule.get('avg_return', 0):.1f}%)")
                    
                    if strategies.get('position_sizing'):
                        pos_size = strategies['position_sizing']
                        recommendations.append("")
                        recommendations.append("POSITION SIZING:")
                        recommendations.append(f"• {pos_size.get('initial_position', '40%')} on initial signal")
                        recommendations.append(f"• {pos_size.get('scale_in_1', '30%')} on first H1 cross")
                        recommendations.append(f"• {pos_size.get('scale_in_2', '30%')} on SMA20 cross above H1")
    ...
    def print_summary(self, top_performers: List[Dict], bottom_performers: List[Dict], analysis: Dict):
        """Print summary of H1/SMA20 scale-in and exit analysis to console"""
        print("\n" + "="*80)
        print(f"BROOKS STRATEGY - H1/SMA20 SCALE-IN AND EXIT ANALYSIS")
        ...
        # H1/SMA20 Scale-in Analysis
        if 'scale_in_exit_patterns' in analysis and analysis['scale_in_exit_patterns']:
            scale_patterns = analysis['scale_in_exit_patterns']
            print("H1/SMA20 SCALE-IN/EXIT PATTERNS:")
            
            if 'scale_in_analysis' in scale_patterns:
                scale_in = scale_patterns['scale_in_analysis']
                print(f"Winners avg scale-ins: {scale_in.get('avg_opportunities_winners', 0):.1f}")
                print(f"Losers avg scale-ins: {scale_in.get('avg_opportunities_losers', 0):.1f}")
            
            if 'optimal_strategies' in scale_patterns:
                strategies = scale_patterns['optimal_strategies']
                if strategies.get('position_sizing'):
                    print("\nRECOMMENDED POSITION SIZING:")
                    pos = strategies['position_sizing']
                    print(f"• Initial: {pos.get('initial_position', 'N/A')}")
                    print(f"• Scale-in 1: {pos.get('scale_in_1', 'N/A')}")
                    print(f"• Scale-in 2: {pos.get('scale_in_2', 'N/A')}")
...
def main():
    ...
    try:
        ...
        parser = argparse.ArgumentParser(description="Brooks Strategy H1/SMA20 Scale-in and Exit Analysis")
        ...
        # Initialize H1/SMA20 analyzer with user credentials
        analyzer = BrooksH1SMA20Analyzer(user_name=args.user, top_n=args.top_n)
        ...
        if results:
            print(f"\nBrooks strategy H1/SMA20 scale-in and exit analysis completed successfully!")
            print(f"Results saved to: {analyzer.results_dir}")
        else:
            print("Brooks strategy H1/SMA20 analysis failed. Check logs for details.")
