#!/usr/bin/env python
"""
Brooks Higher Probability LONG Reversal Strategy Profitability Analysis
======================================================================
This script analyzes the profitability of tickers from Brooks Higher Probability LONG Reversal 
strategy files by date. It tracks performance over different holding periods and provides 
comprehensive statistics for the Al Brooks methodology.
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'logs', 'brooks_strategy_analysis.log'))
    ]
)
logger = logging.getLogger(__name__)

class BrooksStrategyAnalyzer:
    """Analyze profitability of Brooks Higher Probability LONG Reversal strategy"""
    ...
    def __init__(self):
        ...
        logger.info(f"Initialized Brooks Strategy Analyzer")
    ...
    def create_brooks_visualizations(self, all_results: List[Dict], stats: Dict) -> List[str]:
        ...
        try:
            ...
            fig.suptitle('Brooks Strategy Performance Distribution by Holding Period', fontsize=16, fontweight='bold')
            ...
            plt.tight_layout()
            plot_file = os.path.join(self.results_dir, 'brooks_performance_distribution.png')
            ...
            plt.tight_layout()
            plot_file = os.path.join(self.results_dir, 'brooks_win_rates_analysis.png')
            plt.savefig(plot_file, dpi=300, bbox_inches='tight')
            plt.close()
            plot_files.append(plot_file)
            ...
            if 'risk_reward_ratio' in df.columns:
                ...
                plt.tight_layout()
                plot_file = os.path.join(self.results_dir, 'brooks_risk_reward_analysis.png')
                plt.savefig(plot_file, dpi=300, bbox_inches='tight')
                plt.close()
                plot_files.append(plot_file)
    ...
    def export_brooks_excel(self, all_results: List[Dict], stats: Dict) -> str:
        ...
        try:
            ...
            excel_file = os.path.join(self.results_dir, f"brooks_strategy_analysis_{timestamp}.xlsx")
    ...
    def generate_brooks_html_report(self, all_results: List[Dict], stats: Dict, plot_files: List[str], excel_file: str) -> str:
        ...
        try:
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            html_file = os.path.join(self.results_dir, f"brooks_strategy_report_{timestamp}.html")
            ...
            <head>
                ...
                <title>Brooks Higher Probability LONG Reversal Strategy Analysis</title>
                <style>
                    body {{
            ...
            <body>
                <h1>📈 Brooks Higher Probability LONG Reversal Strategy Analysis</h1>
            ...
            if plot_files:
                ...
                for plot_file in plot_files:
                    if os.path.exists(plot_file):
                        plot_name = os.path.basename(plot_file).replace('brooks_', '').replace('_', ' ').replace('.png', '').title()
                        html_content += f"""
            ...
            html_content += f"""
                ...
                <div style="margin-top: 50px; text-align: center; color: #7f8c8d; font-size: 0.9em;">
                    <p>Report generated by Brooks Strategy Analyzer</p>
    ...
    def run_brooks_analysis(self) -> Dict:
        ...
        try:
            logger.info("Starting Brooks Higher Probability LONG Reversal Strategy Analysis")
    ...
    def print_brooks_summary(self, stats: Dict, excel_file: str, html_file: str):
        ...
        print("\\n" + "="*80)
        print("BROOKS HIGHER PROBABILITY LONG REVERSAL STRATEGY ANALYSIS")
        print("="*80)
...
def main():
    ...
    try:
        ...
        analyzer = BrooksStrategyAnalyzer()
        results = analyzer.run_brooks_analysis()
        
        if results:
            print(f"\\nBrooks strategy analysis completed successfully!")
