#!/usr/bin/env python3
"""
User Context Manager for Multi-User Trading System
...
class UserContextManager:
    ...
    def set_current_user(self, user_name: str, credentials: UserCredentials):
        ...
        with self._lock:
            ...
            logger.info(f"Current user set to: {user_name}")
    ...
    def get_user_instance(self, component_type: str, factory_func):
        ...
        with self._lock:
            ...
            if component_type not in user_instances:
                ...
                logger.info(f"Creating new {component_type} instance for user: {self._current_user}")
    ...
    def clear_user_instances(self, user_name: str = None):
        ...
        if target_user and target_user in self._user_instances:
            with self._lock:
                logger.info(f"Clearing instances for user: {target_user}")
                del self._user_instances[target_user]
    
    def list_active_users(self) -> list:
        """Get list of users with active instances"""
        return list(self._user_instances.keys())
...
def get_user_state_manager():
    ...
    current_user = _context_manager.get_current_user()
    if not current_user:
        raise RuntimeError("No current user set")
    
    return _context_manager.get_user_instance(
        'state_manager', 
        lambda: UserAwareStateManager(current_user)
    )
