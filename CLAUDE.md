...
- **2026-02-20**: Fixed pair PnL calculation bug — was computing on gross totals instead of matched qty. A group with 500 hedged/5 unwound showed -2594 instead of -0.30. Fixed to use `matched_qty * (vwap_diff)`. Files: `TG/bot_buy.py`, `TG/bot_sell.py`, `TG/engine.py`.
- **2026-02-20**: Fixed depth cascading guard — groups with multiple D1 targets (from partial fill chunks) now cascade to D2 even after entry completes. Added `d1_count > 1` to `should_cascade` condition. File: `TG/engine.py`.
- **2026-02-20**: Added re-hedging on D2/D4/D6 re-entry fills in depth cascading. Even-depth fills trigger RE-HEDGE, odd-depth closing fills trigger UNWIND. Previously only odd-depth had hedge actions, leaving re-entries unhedged. File: `TG/engine.py`.
- **2026-02-20**: Replaced hard reanchor with soft reanchor. Filled groups stay alive with pending targets; pair hedges accumulate naturally (no forced SPCENET flattening). Only unfilled ENTRY_PENDING entries are cancelled and refreshed at new anchor. Removed `_flatten_pair_position()`. Files: `TG/engine.py`.
- **2026-02-20**: Added circuit breaker `max_position_per_side` — halts new entries when net accumulated position on one side exceeds the configured limit. Checked during reanchor and re-entry. Uses `Group.net_open_qty` (entry fills + even-depth re-entries - odd-depth closes). Files: `TG/{config,group,engine,run,dashboard}.py`, `TG/state/tg_config.json`.
- **2026-02-20**: Added margin/holdings rejection alert banners to Grid dashboard (7777). Engine parses rejection messages (margin shortfall, holdings insufficient, disclosed qty) and stores as alerts in state. Dashboard renders color-coded banners per-symbol with type, side, message, count, and timestamp. Files: `TG/{engine,state,dashboard}.py`.
- **2026-02-20**: Fixed stale order reconciliation on bot restart. Groups with ENTRY_PENDING status and 0 fills whose orders are not found in the broker session are now cleaned up (were blocking new entries on all levels). File: `TG/engine.py`.
- **2026-02-20**: Reduced TATAGOLD qty_per_level from 5000 to 1500 on 01MU07 due to margin constraints (~₹24k available). File: `TG/state/tg_config.json`.
- **2026-02-20**: Multi-account support for TG Grid bot. Two XTS accounts (01MU06, 01MU07) trade simultaneously with isolated state files, PID tracking, XTS sessions, and PnL. Config restructured to `{"accounts": {"01MU06": {...}, "01MU07": {...}}}`. Dashboards (7777, 7779) have account dropdown. PnL dashboard (9000) supports `?account_id=` filter. Files: `TG/{config,state,hybrid_client,engine,run,dashboard}.py`, `TG/pnl/{db_manager,tracker,dashboard}.py`, `TG/state/tg_config.json`.
...
- **2026-02-20**: TollGate dashboard (7788): fixed `computeGrid()` to use per-side buy/sell amounts instead of single uniform amount. Replaced "Recent Transactions" table (was showing only closed groups) with live depth fills table showing all entry/target fills from open groups in real-time — ENTRY, CLOSE (odd depths, with PnL), and RE-ENTRY (even depths) badges. Removed Cumulative PnL chart section. File: `TG/TollGate/dashboard.py`.
- **2026-02-20**: Grid config: updated `qty_per_level` from 100 to 500 for TATSILV and TATAGOLD to increase partial fill probability. Disabled IDEA bot. File: `TG/state/tg_config.json`.
- **2026-02-20**: Ported partial-fill depth cascading from TollGate to TG Grid bot. When an entry partially fills, D1 target is placed for the filled increment; D1 fill spawns D2 re-entry, D2 spawns D3 close, alternating up to `max_sub_depth` (default 10). Moved fill handling from BuyBot/SellBot into engine (matching TollGate architecture). Added `ENTRY_PARTIAL` status, `target_orders` list (replaces single `target_order_id`), backward-compatible state migration. Files: `TG/{config,group,engine,bot_buy,bot_sell,run,dashboard}.py`, `TG/TollGate/config.py`.
- **2026-02-20**: Converted TG Grid `hedge_ratio` and `partial_hedge_ratio` from integer multipliers to percentage (float). Default 40% — if primary leg is 100 qty, hedge is 40 qty on SPCENET. Calculation: `round(qty * ratio / 100)`. Dashboard (7779) labels show "%" suffix, JS uses `parseFloat`. Files: `TG/{config,engine,run,dashboard}.py`, `TG/state/tg_config.json`.
