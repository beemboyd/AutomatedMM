#!/usr/bin/env python3
"""
Long Reversal Pattern Analysis
Analyzes historical performance of Long Reversal Daily signals
"""
...
class LongReversalAnalyzer:
    ...
    def load_all_signals(self):
        ...
        for file_path in sorted(files):
            try:
                ...
                df['Signal_Time'] = date_str[9:15]  # HHMMSS
    ...
    def generate_statistics(self):
        ...
        # Score-wise statistics
        if 'Score' in self.performance_df.columns:
            score_stats = {}
            for score in self.performance_df['Score'].unique():
                score_data = self.performance_df[self.performance_df['Score'] == score]
                score_stats[str(score)] = {
                    'Count': len(score_data),
                    'Avg_3D_Return': score_data['Return_3D'].mean() if 'Return_3D' in score_data else None,
                    'Win_Rate_3D': (score_data['Return_3D'] > 0).sum() / len(score_data) * 100 
                                   if 'Return_3D' in score_data else None
                }
    ...
    def save_results(self):
        ...
        if hasattr(self, 'performance_df'):
            output_file = f'/Users/maverick/PycharmProjects/India-TS/Daily/analysis/long_reversal_performance_{timestamp}.xlsx'
        ...
        if stats:
            stats_file = f'/Users/maverick/PycharmProjects/India-TS/Daily/analysis/long_reversal_stats_{timestamp}.json'
            with open(stats_file, 'w') as f:
                json.dump(stats, f, indent=4, default=str)
            logger.info(f"Statistics saved to {stats_file}")
            
            # Print summary
            print("\n=== LONG REVERSAL PATTERN ANALYSIS SUMMARY ===")
...
def main():
    analyzer = LongReversalAnalyzer()
    
    # Run analysis on recent signals (last 200)
    analyzer.run_analysis(sample_size=200)
