...
def load_daily_config(user_name="Sai"):
    ...
    if not os.path.exists(config_path):
        print(f"config.ini file not found at {config_path}")
    ...
    if credential_section not in config.sections():
        print(f"No credentials found for user {user_name} in {config_path}")
...
# Initialize Kite connection using user-specific credentials
user_name = "Sai"  # You can change this if needed
...
print(f"\nUsing credentials for user: {user_name}")
...
for idx, row in top_10_tickers.iterrows():
    ...
    try:
        ...
        ticker_info = instruments_df[instruments_df['tradingsymbol'] == ticker]
        if ticker_info.empty:
            print(f"Instrument token not found for {ticker}")
            continue
            
        instrument_token = int(ticker_info.iloc[0]['instrument_token'])
        ...
        # Get entry price (use provided entry price from scan)
        ...
        if not exit_day_data.empty:
            ...
            exit_date_actual = exit_day_data.iloc[0]['date']
        else:
            # Use last available day
            if not hist_df.empty:
                exit_price = hist_df.iloc[-1]['close']
                exit_date_actual = hist_df.iloc[-1]['date']
            else:
                continue
        ...
        sl_hit = hist_df['low'].min() <= stop_loss
        
        # If stop loss was hit, find the day and adjust exit price
        if sl_hit:
            sl_days = hist_df[hist_df['low'] <= stop_loss]
            if not sl_days.empty:
                sl_day = sl_days.iloc[0]
