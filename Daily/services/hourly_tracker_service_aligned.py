#!/usr/bin/env python3
"""
Hourly Tracker Service - Aligned with Daily VSR Tracker
Tracks Long_Reversal_Hourly tickers using the same scoring logic as VSR tracker
"""
...
class HourlyTrackerServiceAligned:
    def __init__(self, user_name: str = 'Sai', interval: int = 60):
        ...
        self.kite = initialize_kite(config)
        
        # Track previous results for momentum build
        self.previous_results = {}
        
        # Initialize persistence data (matching daily VSR structure)
        ...
        self.logger.info(f"[{user_name}] Hourly Tracker Service (Aligned) initialized")
    ...
    def setup_logging(self):
        ...
        log_file = os.path.join(log_dir, f'hourly_tracker_aligned_{datetime.date.today().strftime("%Y%m%d")}.log')
        
        # Create logger
        self.logger = logging.getLogger('HourlyTrackerAligned')
    ...
    def track_ticker_vsr(self, ticker: str) -> Optional[Dict]:
        ...
        try:
            ...
            # Fetch minute data for real-time price/volume
            ...
            # Add technical indicators to minute data using calculate_vsr_indicators
            ...
            # Calculate 5-minute momentum from minute data
            ...
            # Get latest indicators from minute data
            ...
            # Check for momentum build (using minute data)
            momentum_build = 0
            if len(minute_with_indicators) >= 15:  # 15 minutes
                ...
                # Check if VSR is building (every 5 minutes)
            ...
            # Calculate score (0-100) - Same logic as daily VSR tracker
            score = 0
            ...
            if len(ticker_data['scores']) > 48:  # Keep last 48 hours of hourly scores
    ...
    def run_scan(self):
        ...
        try:
            self.logger.info("="*80)
            self.logger.info(f"[{self.user_name}] Starting Hourly tracker scan (Aligned) at {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
            ...
            # Clean up old tickers from persistence (not seen in last 3 days)
    ...
    def run(self):
        """Main run loop"""
        self.logger.info(f"[{self.user_name}] Hourly Tracker Service (Aligned) started")
...
def main():
    parser = argparse.ArgumentParser(description='Hourly Tracker Service (Aligned with Daily VSR)')
    parser.add_argument('--user', type=str, default='Sai', help='User name for tracking')
    parser.add_argument('--interval', type=int, default=60, help='Scan interval in seconds')
    
    args = parser.parse_args()
    
    service = HourlyTrackerServiceAligned(args.user, args.interval)
    service.run()
