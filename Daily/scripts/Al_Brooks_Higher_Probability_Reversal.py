#!/usr/bin/env python
# Al_Brooks_Higher_Probability_Reversal.py - Filter stocks based on higher probability reversal criteria:
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                                       "logs", "al_brooks_higher_probability.log")),
...
def parse_args():
    parser = argparse.ArgumentParser(description="Al Brooks Higher Probability Reversal Analysis")
...
def generate_html_report(filtered_df, output_file, scanner_file):
    ...
    <head>
        ...
        <title>Higher Probability Reversal Filter - {formatted_date}</title>
        <style>
            body {{
    ...
    <body>
        <h1>Higher Probability LONG Reversal Filter</h1>
        <div class="header-info">
            <div>Date: {formatted_date} | Time: {formatted_time}</div>
            <div>Filtered from: Ticker.xlsx | LONG ONLY | Higher Probability: 60%+</div>
        </div>

        <h2>Higher Probability LONG Reversal Patterns ({len(filtered_df)} matches)</h2>
    """
    ...
    for idx, row in filtered_df.iterrows():
        ...
        html_content += f"""
            <tr>
                <td>{row['Ticker']}</td>
                <td><span class="ticker-direction {direction_class}">{row['Direction']}</span></td>
                <td>{row['Score']}</td>
    ...
    for idx, row in filtered_df.iterrows():
        direction_class = row['Direction'].lower()
        html_content += f"""
        <div class="ticker-card {direction_class}">
            <div class="ticker-header">
                <div class="ticker-name">{row['Ticker']}</div>
                <div>
                    <span class="ticker-direction {direction_class}">{row['Direction']}</span>
    ...
    html_content += f"""
        <div class="source-info">
            <p>Generated on {formatted_date} at {formatted_time} | Al Brooks Higher Probability Reversal Filter</p>
...
# -----------------------------
def main():
    """Main function to filter tickers for higher probability LONG reversals"""
    logger.info("Higher Probability LONG Reversal filter")
    ...
    try:
        ...
        excel_file = os.path.join(RESULTS_DIR, f"Brooks_Higher_Probability_LONG_Reversal_{formatted_date}_{formatted_time}.xlsx")
        html_file = os.path.join(HTML_DIR, f"Higher_Probability_LONG_Analysis_{formatted_date}_{formatted_time.replace('_', '-')}.html")
        ...
        if results:
            ...
            print("\n===== Al Brooks Higher Probability LONG Reversal Results =====")
            print(f"Found {len(results_df)} higher probability LONG reversal patterns")
            print("\nTop 5 LONG patterns by score:")
            for idx, row in results_df.head(5).iterrows():
                print(f"{row['Ticker']} (LONG): Score {row['Score']}, Entry ₹{row['Entry_Price']:.2f}, SL ₹{row['Stop_Loss']:.2f}, R:R {row['Risk_Reward_Ratio']:.2f}")

            print(f"\nDetailed results saved to: {excel_file}")
            print(f"HTML report opened in browser: {html_file}")
        else:
            # Create empty Excel with columns
            empty_cols = ['Ticker', 'Pattern', 'Direction', 'Score', 'Entry_Price', 'Stop_Loss', 'Target1', 'Target2', 
            ...
            <head>
                <meta charset="UTF-8">
                <meta name="viewport" content="width=device-width, initial-scale=1.0">
                <title>Al Brooks Higher Probability LONG Reversal Filter - {formatted_date}</title>
                <style>
                    body {{
                        font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
                        line-height: 1.6;
            ...
            <body>
                <h1>📈 Al Brooks Higher Probability LONG Reversal Filter</h1>
                ...
                <div style="margin-top: 50px; color: #999;">
                    <p>Generated on {formatted_date} at {formatted_time.replace('_', ':')} | Al Brooks Higher Probability LONG Filter</p>
                </div>
            </body>
            </html>
            """
...
if __name__ == "__main__":
    # Print banner
    print("\n===================================")
    print("Higher Probability Reversal Filter")
    print("===================================")
    ...
    print("3. Break of significant support/resistance")
