...
Monitors Short Reversal Hourly scanner results with VSR analysis
Port: 3004
"""
...
LOGS_DIR = os.path.join(BASE_DIR, 'logs', 'hourly_short_tracker')
DATA_DIR = os.path.join(BASE_DIR, 'data', 'short_momentum')

def parse_hourly_logs(hours=2):
    """Parse hourly short tracker logs to extract trending tickers"""
    trending_tickers = defaultdict(list)
    
    # Get current date log file
    today = datetime.datetime.now().strftime("%Y%m%d")
    log_file = os.path.join(LOGS_DIR, f'hourly_short_tracker_{today}.log')
...
def categorize_tickers(latest_data):
    ...
    for ticker, data in latest_data.items():
        ...
        # For shorts, strong negative momentum is good
        if data['momentum'] <= -5:
...
def load_state_file():
    """Load the latest state from hourly short tracker"""
    state_file = os.path.join(DATA_DIR, 'hourly_short_tracker_state.json')
...
def index():
    ...
    return render_template('hourly_short_tracker_dashboard.html')
...
def get_hourly_persistence():
    ...
    try:
        persistence_file = os.path.join(DATA_DIR, 'vsr_ticker_persistence_hourly_short.json')
...
if __name__ == '__main__':
    print("Starting Hourly Short Tracker Dashboard on port 3004...")
    print("Access the dashboard at: http://localhost:3004")
