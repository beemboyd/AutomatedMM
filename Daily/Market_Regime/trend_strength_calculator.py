...
class TrendStrengthCalculator:
    ...
    def load_latest_scan(self):
        ...
        if not long_files or not short_files:
            ...
            try:
                with open(latest_file, 'r') as f:
                    data = json.load(f)
                logger.info(f"Loaded cached scan results from {latest_file}")
                return data
        ...
        latest_long = max(long_files, key=os.path.getmtime) if long_files else None
        latest_short = max(short_files, key=os.path.getmtime) if short_files else None
        
        try:
            # Count rows in Excel files
            long_count = 0
            short_count = 0
            
            if latest_long:
                df_long = pd.read_excel(latest_long)
                long_count = len(df_long)
                
            if latest_short:
                df_short = pd.read_excel(latest_short)
            ...
            data = {
                ...
                'long_file': latest_long,
                'short_file': latest_short
            }
            
            logger.info(f"Loaded scan results directly from Excel: Long={long_count}, Short={short_count}")
            return data
        except Exception as e:
    ...
    def generate_trend_report(self, scan_data, historical_data=None):
        ...
        # Market score: normalized ratio between -1 and 1
        if ratio == float('inf'):
            market_score = 1.0
        elif ratio == 0:
            market_score = -1.0
        ...
        report = {
            ...
            'trend_strength': {
                ...
                'strength': abs(market_score)  # Absolute strength for confidence calculation
            },
