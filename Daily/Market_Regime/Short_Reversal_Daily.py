...
if os.path.basename(SCRIPT_DIR_TEMP) == "Market_Regime":
    LOG_DIR = os.path.join(SCRIPT_DIR_TEMP, "logs")
else:
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(LOG_DIR, "short_reversal_daily.log")),
...
# Check if we're running from Market_Regime folder
if os.path.basename(SCRIPT_DIR) == "Market_Regime":
    DATA_DIR = os.path.join(os.path.dirname(os.path.dirname(SCRIPT_DIR)), "data")
    RESULTS_DIR = os.path.join(SCRIPT_DIR, "results")  # Use same results dir in Market_Regime
    HTML_DIR = os.path.join(SCRIPT_DIR, "results")
else:
