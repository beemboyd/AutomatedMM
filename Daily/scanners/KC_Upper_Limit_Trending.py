#!/usr/bin/env python
# KC_Upper_Limit_Trending.py - Filter stocks based on Keltner Channel upper limit breakout with trend following:
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                                       "logs", "kc_upper_limit_trending.log")),
...
def parse_args():
    parser = argparse.ArgumentParser(description="KC Upper Limit Trending Analysis with Sector Information")
...
PDF_DIR = os.path.join(os.path.dirname(SCRIPT_DIR), "results", "PDF")
...
def generate_html_report(filtered_df, output_file, scanner_file):
    ...
    <head>
        ...
        <title>KC Upper Limit Trending Filter - {formatted_date}</title>
        <style>
            body {{
    ...
    <body>
        <h1>Keltner Channel Upper Limit Trending Filter</h1>
        <div class="header-info">
            <div>Date: {formatted_date} | Time: {formatted_time}</div>
            <div>Filtered from: Ticker.xlsx | Daily Timeframe | Trend Following Strategy</div>
        </div>

        <div class="kc-info">
            <h3>Strategy Overview</h3>
            <p>This scanner identifies high-probability trading opportunities using:</p>
            <ul>
                <li><strong>H2 Pattern:</strong> Two consecutive bars closing above previous highs (momentum confirmation)</li>
                <li><strong>KC Breakouts:</strong> Price breaking above Keltner Channel upper band</li>
                <li><strong>Volume Surge:</strong> Increased volume on H2 bars shows institutional interest</li>
                <li><strong>Pattern Combos:</strong> H2 + KC + Volume = Highest probability setups</li>
                <li><strong>Early Detection:</strong> Identifies stocks building strength before major moves</li>
            </ul>
        </div>

        <h2>High Probability Setups ({len(filtered_df)} matches)</h2>
    """
    ...
    if len(h2_kc_combos) > 0:
        html_content += """
        <h3 style="color: #e74c3c;">🔥 H2 + KC Combos (HIGHEST PROBABILITY)</h3>
        <p>These stocks show both H2 pattern AND KC breakout - ideal for position building!</p>
        """
    elif len(h2_patterns) > 0:
        html_content += """
        <h3 style="color: #f39c12;">📈 H2 Patterns Detected</h3>
        <p>These stocks show H2 momentum patterns - watch for continuation!</p>
        """
        ...
        <div class="ticker-card">
            ...
            <div class="pattern-info">
                ...
                <strong>Key Insight:</strong> {
                    "H2 + KC combo detected - HIGHEST probability setup!" if row['Pattern'] == 'H2_KC_Combo' else
                    "H2 with volume surge - Strong momentum building!" if row['Pattern'] == 'H2_Volume_Surge' else
                    "Multiple H2 patterns above KC - Position building opportunity!" if row['Pattern'] == 'KC_Multi_H2' else
                    "H2 pattern forming - Watch for continuation!" if row['Has_H2'] else
                    "Near KC upper band - Monitor for H2 development"
                }
                <br>
    ...
    html_content += f"""
        <div class="source-info">
            <p>Generated on {formatted_date} at {formatted_time} | KC Upper Limit Trending Filter</p>
            <p><strong>Note:</strong> These setups identify stocks breaking out above Keltner Channel upper band with strong trending characteristics.</p>
            <p><strong>Risk Management:</strong> Use proper position sizing. Stocks with lower congestion days and higher trend strength typically perform better.</p>
        </div>
    </body>
    </html>
    """
...
def generate_pdf_report(filtered_df, output_file):
    ...
    try:
        ...
        # Title
        story.append(Paragraph("KC Upper Limit Trending Filter", title_style))
        ...
        overview_text = """This scanner identifies high-probability trading opportunities using:
        • H2 Pattern: Two consecutive bars closing above previous highs
        • KC Breakouts: Price breaking above Keltner Channel upper band
        • Volume Surge: Increased volume on H2 bars shows institutional interest
        • Pattern Combos: H2 + KC + Volume = Highest probability setups
        • Early Detection: Identifies stocks building strength before major moves"""
        story.append(Paragraph(overview_text.replace('\n', '<br/>'), normal_style))
        story.append(Spacer(1, 0.3*inch))
        
        # Summary
        story.append(Paragraph(f"High Probability Setups ({len(filtered_df)} matches)", heading_style))
        ...
        if len(filtered_df) > 0:
            ...
            story.append(Paragraph("No KC upper trend patterns found in today's scan.", normal_style))
        ...
        story.append(Paragraph(f"Generated on {formatted_date} at {formatted_time} | KC Upper Limit Trending Filter", footer_style))
        story.append(Paragraph("Risk Management: Use proper position sizing. Stocks with lower congestion days typically perform better.", footer_style))
...
# -----------------------------
def main():
    """Main function to filter tickers for KC upper limit trending patterns"""
    logger.info("KC Upper Limit Trending filter with Sector Information")
    ...
    try:
        ...
        excel_file = os.path.join(RESULTS_DIR, f"KC_Upper_Limit_Trending_{formatted_date}_{formatted_time}.xlsx")
        html_file = os.path.join(HTML_DIR, f"KC_Upper_Limit_Trending_{formatted_date}_{formatted_time.replace('_', '-')}.html")
        pdf_file = os.path.join(PDF_DIR, f"KC_Upper_Limit_Trending_{formatted_date}_{formatted_time}.pdf")
        ...
        if results:
            ...
            try:
                ...
                market_regime_file = os.path.join(market_regime_dir, f"KC_Upper_Limit_Trending_{formatted_date}_{formatted_time}.xlsx")
            ...
            print(f"Found {len(results_df)} high probability setups")
            ...
            if len(g_patterns) > 0:
                print(f"\n💎 G PATTERNS ({len(g_patterns)} stocks) - TIME TO BUILD/DOUBLE POSITIONS:")
            ...
            if len(h2_kc_combos) > 0:
                print(f"\n🔥 STRONG H2 + KC COMBOS ({len(h2_kc_combos)} stocks) - HIGH PROBABILITY:")
            ...
            if len(h2_patterns) > 0:
                print(f"\n📈 H2 Patterns Detected ({len(h2_patterns)} stocks):")
            ...
            if len(high_prob) > 0:
                print(f"\n⭐ High Probability Setups (Score >= 70): {len(high_prob)} stocks")
            ...
            print("\nTop 5 setups by probability score:")
            ...
            <body>
                <h1>📈 KC Upper Limit Trending Filter</h1>
                <div class="no-results">
                    <h2>No KC Upper Trend Patterns Found</h2>
                    <p>No tickers matched the Keltner Channel upper limit trending criteria in today's scan.</p>
                ...
                <div style="margin-top: 50px; color: #999;">
                    <p>Generated on {formatted_date} at {formatted_time.replace('_', ':')} | KC Upper Limit Trending Filter</p>
                </div>
            </body>
            </html>
            """
            ...
            logger.info(f"No KC upper trend patterns found. Empty output files created at {excel_file}, {html_file}, and {pdf_file}")
            print("\nNo KC upper trend patterns found.")
...
if __name__ == "__main__":
    # Print banner
    print("\n===================================")
    print("High Probability Pattern Scanner")
    print("===================================")
    print("Identifying G-Pattern setups:")
    ...
    print("- Initial: H2 pattern forms")
    print("- Double: Volume surge on H2 (day 3-5)")
    print("- Hold: For multi-week moves")
    print("===================================")
