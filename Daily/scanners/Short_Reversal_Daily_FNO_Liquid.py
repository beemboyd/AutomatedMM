#!/usr/bin/env python
# Short_Reversal_Daily_FNO_Liquid.py - Filter FNO Liquid stocks based on higher probability reversal criteria with Sector information:
# 1. Wait for strong breakdown in new direction (confirmed reversal)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                                       "logs", "short_reversal_fno_liquid.log")),
...
def parse_args():
    parser = argparse.ArgumentParser(description="Short Reversal Daily Analysis for FNO Liquid stocks with Sector Information")
...
# The rest of the functions remain the same as Short_Reversal_Daily.py
# but will use FNO_Liquid.xlsx instead of Ticker.xlsx

from Daily.scanners.Short_Reversal_Daily import (
    ...
    create_detailed_analysis_report, process_ticker, analyze_shorts
)
...
if __name__ == "__main__":
    ...
    # Output paths - use FNO/Short/Liquid directory
    output_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 
                              "FNO", "Short", "Liquid")
    os.makedirs(output_dir, exist_ok=True)
    
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    excel_path = os.path.join(output_dir, f"Short_Reversal_Daily_FNO_{timestamp}.xlsx")
    html_path = os.path.join(output_dir, f"Short_Reversal_Daily_FNO_{timestamp}.html")
    ...
    try:
        ...
        # Run analysis
        logger.info("Starting Short Reversal analysis for FNO Liquid stocks...")
        results = analyze_shorts(api_key, tickers_df, user=user)
        ...
        if results:
            ...
            logger.info("No stocks met the Short Reversal criteria for FNO Liquid stocks")
