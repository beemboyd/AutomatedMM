#!/usr/bin/env python
# Short_Reversal_Daily_FNO_Liquid.py - Filter FNO Liquid stocks based on higher probability SHORT reversal criteria with Sector information:
...
def parse_args():
    parser = argparse.ArgumentParser(description="Short Reversal Daily Analysis for FNO Liquid stocks with Al Brooks L2 Patterns")
...
HTML_DIR = os.path.join(os.path.dirname(SCRIPT_DIR), "FNO", "Short", "Liquid")
...
def generate_html_report(filtered_df, output_file, scanner_file):
    ...
    <head>
        ...
        <title>Short Reversal Daily FNO Liquid Filter - {formatted_date}</title>
        <style>
            body {{
    ...
    <body>
        <h1>📉 Short Reversal Daily FNO Liquid Filter - Al Brooks L2 Patterns</h1>
        <div class="header-info">
            <div>Date: {formatted_date} | Time: {formatted_time}</div>
            <div>Filtered from: FNO_Liquid.xlsx | SHORT ONLY | L2 & Bearish Patterns</div>
        </div>
...
# -----------------------------
def main():
    """Main function to filter FNO Liquid tickers for high probability SHORT patterns"""
    logger.info("Short Reversal Daily FNO Liquid filter with Al Brooks L2 Patterns")
...
if __name__ == "__main__":
    # Print banner
    print("\n=====================================")
    print("Short Reversal Daily FNO Liquid Filter")
    print("==================================")
