...
SECTOR_MAPPING = {
    ...
    'MUTHOOTFIN': 'Financial Services', 'CHOLAFIN': 'Financial Services', 'LICHSGFIN': 'Financial Services',
    'ICICIGI': 'Insurance', 'SBILIFE': 'Insurance', 'HDFCLIFE': 'Insurance', 'ICICIPRULI': 'Insurance',
    
    # Information Technology
    'TCS': 'Information Technology', 'INFY': 'Information Technology', 'WIPRO': 'Information Technology',
    'HCLTECH': 'Information Technology', 'TECHM': 'Information Technology', 'LTTS': 'Information Technology',
    'MPHASIS': 'Information Technology', 'COFORGE': 'Information Technology', 'PERSISTENT': 'Information Technology',
    'MINDTREE': 'Information Technology', 'LTIM': 'Information Technology',
    
    # Pharmaceuticals & Healthcare
    'SUNPHARMA': 'Pharmaceuticals', 'DRREDDY': 'Pharmaceuticals', 'CIPLA': 'Pharmaceuticals',
    'DIVISLAB': 'Pharmaceuticals', 'BIOCON': 'Pharmaceuticals', 'AUROPHARMA': 'Pharmaceuticals',
    'LUPIN': 'Pharmaceuticals', 'TORNTPHARM': 'Pharmaceuticals', 'ALKEM': 'Pharmaceuticals',
    'APOLLOHOSP': 'Healthcare', 'FORTIS': 'Healthcare', 'MAXHEALTH': 'Healthcare',
    
    # FMCG
    'HINDUNILVR': 'FMCG', 'ITC': 'FMCG', 'NESTLEIND': 'FMCG', 'BRITANNIA': 'FMCG',
    'DABUR': 'FMCG', 'MARICO': 'FMCG', 'GODREJCP': 'FMCG', 'COLPAL': 'FMCG',
    'TATACONSUM': 'FMCG', 'VBL': 'FMCG', 'PGHH': 'FMCG', 'EMAMILTD': 'FMCG',
    
    # Automobiles
    'TATAMOTORS': 'Automobiles', 'M&M': 'Automobiles', 'MARUTI': 'Automobiles',
    'BAJAJ-AUTO': 'Automobiles', 'HEROMOTOCO': 'Automobiles', 'EICHERMOT': 'Automobiles',
    'ASHOKLEY': 'Automobiles', 'TVSMOTORS': 'Automobiles', 'MRF': 'Auto Ancillaries',
    'APOLLOTYRE': 'Auto Ancillaries', 'CEAT': 'Auto Ancillaries', 'MOTHERSON': 'Auto Ancillaries',
    
    # Oil & Gas
    'RELIANCE': 'Oil & Gas', 'ONGC': 'Oil & Gas', 'IOC': 'Oil & Gas', 'BPCL': 'Oil & Gas',
    'HINDPETRO': 'Oil & Gas', 'GAIL': 'Oil & Gas', 'PETRONET': 'Oil & Gas', 'IGL': 'Oil & Gas',
    'MGL': 'Oil & Gas', 'CASTROLIND': 'Oil & Gas',
    
    # Metals & Mining
    'TATASTEEL': 'Metals & Mining', 'HINDALCO': 'Metals & Mining', 'JSWSTEEL': 'Metals & Mining',
    'VEDL': 'Metals & Mining', 'NMDC': 'Metals & Mining', 'SAIL': 'Metals & Mining',
    'JINDALSTEL': 'Metals & Mining', 'NATIONALUM': 'Metals & Mining', 'HINDZINC': 'Metals & Mining',
    ...
    'ULTRACEMCO': 'Cement', 'SHREECEM': 'Cement', 'AMBUJACEM': 'Cement', 'ADANICEMENT': 'Cement',
    'RAMCOCEM': 'Cement', 'DALMIACEM': 'Cement', 'JKCEMENT': 'Cement', 'BIRLACORPN': 'Cement',
    'GRASIM': 'Cement', 'ACC': 'Cement',
    ...
    'SOBHA': 'Real Estate', 'LODHA': 'Real Estate',
    
    # Telecom
    'BHARTIARTL': 'Telecom', 'JIO': 'Telecom', 'IDEA': 'Telecom', 'TTML': 'Telecom',
    
    # Retail
    'TITAN': 'Retail', 'DMART': 'Retail', 'TRENT': 'Retail', 'SHOPERSTOP': 'Retail',
    'JUBLFOOD': 'Retail', 'WESTLIFE': 'Retail', 'VMART': 'Retail', 'ABFRL': 'Retail',
    
    # Infrastructure & Construction
    'LARSENTOUBRO': 'Infrastructure', 'ADANIENT': 'Infrastructure', 'ADANIPORTS': 'Infrastructure',
    'CONCOR': 'Infrastructure', 'IRCTC': 'Infrastructure', 'INDIANB': 'Infrastructure',
    'KEC': 'Infrastructure', 'NCC': 'Infrastructure', 'NBCC': 'Infrastructure',
    
    # Chemicals
    'PIDILITIND': 'Chemicals', 'ASIANPAINT': 'Chemicals', 'BERGEPAINT': 'Chemicals',
    'SRF': 'Chemicals', 'ATUL': 'Chemicals', 'GALAXYSURF': 'Chemicals', 'DEEPAKNTR': 'Chemicals',
    'TATACHEM': 'Chemicals', 'UPL': 'Chemicals', 'AARTI': 'Chemicals',
    
    # Textiles
    'PAGEIND': 'Textiles', 'ARVIND': 'Textiles', 'WELSPUNIND': 'Textiles', 'TRIDENT': 'Textiles',
    'VARDHACRLC': 'Textiles', 'RAYMOND': 'Textiles', 'KEWALRAM': 'Textiles',
    
    # Media & Entertainment
    'ZEEL': 'Media & Entertainment', 'SUNTV': 'Media & Entertainment', 'TV18BRDCST': 'Media & Entertainment',
    'NETWORK18': 'Media & Entertainment', 'PVRINOX': 'Media & Entertainment',
...
def get_sector(ticker):
    """Get sector for a given ticker, returns 'Others' if not found"""
    # Clean the ticker - remove any suffixes or special characters
    clean_ticker = ticker.strip().upper()
    
    # Direct lookup
    if clean_ticker in SECTOR_MAPPING:
        return SECTOR_MAPPING[clean_ticker]
    
    # Try without common suffixes
    base_ticker = clean_ticker.split('-')[0].split('_')[0]
    if base_ticker in SECTOR_MAPPING:
        return SECTOR_MAPPING[base_ticker]
    
    # Check if it's an index
    if 'NIFTY' in clean_ticker:
        return 'Index'
...
<head>
    ...
    <style>
        ...
        .sector-badge {{ background-color: #9b59b6; color: white; padding: 4px 8px; border-radius: 4px; font-size: 0.75em; margin-left: 10px; }}
        .sector-section {{ margin: 30px 0; background-color: #ecf0f1; padding: 15px; border-radius: 8px; }}
        .sector-title {{ font-size: 1.2em; font-weight: bold; color: #2c3e50; margin-bottom: 10px; }}
...
"""
        ...
        if not filtered_df.empty:
            ...
            sectors = filtered_df.groupby('Sector')
            
            for sector_name, sector_stocks in sectors:
                html_content += f'''<div class="sector-section">
                    <div class="sector-title">📊 {sector_name} ({len(sector_stocks)} stocks)</div>
...
def main():
    ...
    try:
        ...
        if results:
            ...
            # Sort by sector first, then by strength score (highest first)
            df = df.sort_values(['Sector', 'Strength_Score'], ascending=[True, False])
            ...
            with pd.ExcelWriter(excel_file, engine='openpyxl') as writer:
                ...
                # Create sector-wise sheets
                for sector in df['Sector'].unique():
                    sector_df = df[df['Sector'] == sector]
                    sheet_name = sector[:30] if len(sector) > 30 else sector  # Excel sheet name limit
                ...
                for sector in sorted(df['Sector'].unique()):
                    ...
                    summary_data.append({
                        'Sector': sector,
                        'Count': len(sector_df),
                        'Avg_Strength_Score': round(sector_df['Strength_Score'].mean(), 1),
                        'Avg_Risk_Reward': round(sector_df['Risk_Reward_Ratio'].mean(), 1),
                        'Top_Stock': sector_df.iloc[0]['Ticker'] if len(sector_df) > 0 else 'N/A',
                        'Top_Stock_Strength': sector_df.iloc[0]['Strength_Score'] if len(sector_df) > 0 else 0
                    })
            ...
            for sector, count in sector_counts.items():
                sector_df = df[df['Sector'] == sector]
                logger.info(f"  {sector}: {count} stocks (Avg Strength: {sector_df['Strength_Score'].mean():.1f})")

            # Show top results
            logger.info(f"\nTop 5 weekly candidates by strength:")
            top_5 = df.nlargest(5, 'Strength_Score')
            for i, row in top_5.iterrows():
                logger.info(f"  {row['Ticker']} ({row['Sector']}): Entry ₹{row['Entry_Price']:.2f}, R:R 1:{row['Risk_Reward_Ratio']:.1f}, Strength {row['Strength_Score']:.1f}")
            ...
            empty_df = pd.DataFrame(columns=['Ticker', 'Sector', 'Entry_Price', 'Stop_Loss', 'Target_1', 'Target_2', 'Risk_Reward_Ratio', 'Strength_Score', 'Timeframe'])
