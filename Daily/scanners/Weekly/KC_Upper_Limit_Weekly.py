#!/usr/bin/env python
# KC_Upper_Limit_Weekly.py - Filter stocks based on Keltner Channel upper limit breakout with trend following:
# 1. Stock must be trading above Keltner Channel upper band (weekly timeframe)
# 2. Strong trending behavior (avoiding congestion/ranging stocks)
# 3. Volume confirmation on breakout
# 4. Momentum confirmation
# 5. Weekly timeframe focus
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
                                       "logs", "kc_upper_limit_weekly.log")),
...
def parse_args():
    parser = argparse.ArgumentParser(description="KC Upper Limit Weekly Analysis with Sector Information")
...
PDF_DIR = os.path.join(os.path.dirname(os.path.dirname(SCRIPT_DIR)), "Detailed_Analysis", "PDF")
    ...
    if cache.instruments_df is None:
        try:
            ...
            if instruments:
                ...
                try:
                    backup_file = os.path.join(os.path.dirname(os.path.dirname(SCRIPT_DIR)), "data", "instruments_backup.csv")
...
# -----------------------------
# Calculate Indicators for Keltner Channel Analysis (Weekly)
# -----------------------------
def calculate_indicators(weekly_data):
    """Calculate indicators for Keltner Channel trend detection on weekly timeframe"""
    ...
    # Calculate EMAs for Keltner Channel (using similar periods for weekly)
    ...
    # Calculate trend strength indicators (adjusted for weekly)
    df['SMA10'] = df['Close'].rolling(window=10).mean()  # 10-week SMA ~ 50-day
    df['SMA40'] = df['Close'].rolling(window=40).mean()  # 40-week SMA ~ 200-day
    ...
    # Calculate congestion/ranging detection (adjusted for weekly)
    lookback_weeks = 10  # 10 weeks instead of 20 days
    df['High_10W'] = df['High'].rolling(window=lookback_weeks).max()
    df['Low_10W'] = df['Low'].rolling(window=lookback_weeks).min()
    df['Range_10W'] = df['High_10W'] - df['Low_10W']
    df['Range_Percent'] = (df['Range_10W'] / df['Close']) * 100
    
    # Method 2: Count weeks within narrow band
    narrow_band_pct = 10.0  # 10% band for weekly (wider than daily)
    df['In_Narrow_Band'] = (df['Range_Percent'] < narrow_band_pct).astype(int)
    df['Congestion_Weeks'] = df['In_Narrow_Band'].rolling(window=lookback_weeks).sum()
    
    # Method 3: Keltner Channel squeeze (bands getting narrower)
    df['KC_Width'] = df['KC_Upper'] - df['KC_Lower']
    df['KC_Width_MA'] = df['KC_Width'].rolling(window=10).mean()
    df['KC_Squeeze'] = df['KC_Width'] < df['KC_Width_MA']
    
    # Volume indicators (adjusted for weekly)
    df['AvgVolume10'] = df['Volume'].rolling(window=10).mean()
    df['VolumeRatio'] = df['Volume'] / df['AvgVolume10']
    
    # Momentum indicators (adjusted for weekly)
    df['ROC2'] = ((df['Close'] - df['Close'].shift(2)) / df['Close'].shift(2)) * 100  # 2-week momentum
    df['ROC4'] = ((df['Close'] - df['Close'].shift(4)) / df['Close'].shift(4)) * 100  # 4-week momentum
    df['ROC8'] = ((df['Close'] - df['Close'].shift(8)) / df['Close'].shift(8)) * 100  # 8-week momentum
    
    # Trend consistency (how many of last N weeks were above KC upper)
    df['Weeks_Above_KC'] = df['Above_KC_Upper'].rolling(window=5).sum()
    ...
    # Weeks since last breakout
    df['Breakout_Signal'] = df['Fresh_Breakout'].astype(int)
    df['Weeks_Since_Breakout'] = 0
    for i in range(1, len(df)):
        if df.iloc[i]['Breakout_Signal'] == 1:
            df.iloc[i, df.columns.get_loc('Weeks_Since_Breakout')] = 0
        else:
            df.iloc[i, df.columns.get_loc('Weeks_Since_Breakout')] = df.iloc[i-1]['Weeks_Since_Breakout'] + 1
    
    # Calculate trend score
    df['Trend_Score'] = 0
    # Add points for trend alignment
    df.loc[df['Close'] > df['SMA10'], 'Trend_Score'] += 1
    df.loc[df['Close'] > df['SMA40'], 'Trend_Score'] += 1
    df.loc[df['SMA10'] > df['SMA40'], 'Trend_Score'] += 1
    df.loc[df['ADX'] > 25, 'Trend_Score'] += 1
    df.loc[df['ROC8'] > 10, 'Trend_Score'] += 1
    
    # H2 Pattern Detection (Two consecutive weeks closing above previous high)
    df['H2_Bar'] = False
    df['H2_Count'] = 0
    ...
    for i in range(2, len(df)):
        # H2: Current week closes above previous week's high AND previous week closed above its previous high
    ...
    df['H2_Weeks_Recent'] = 0  # Count H2 patterns in last 5 weeks
    df['Volume_Surge_Weeks'] = 0  # Count volume surge weeks in last 5 weeks
    df['Building_Momentum'] = False  # Flag for weekly momentum patterns
    
    for i in range(5, len(df)):
        # Check if H2 occurred within 3 weeks of KC breakout
        if df.iloc[i]['H2_Bar']:
            # Look back 3 weeks for KC breakout
        ...
        # Count H2 patterns in last 5 weeks
        h2_recent_count = df.iloc[i-4:i+1]['H2_Bar'].sum()
        df.iloc[i, df.columns.get_loc('H2_Weeks_Recent')] = h2_recent_count
        
        # Count volume surge weeks in last 5 weeks
        vol_surge_count = (df.iloc[i-4:i+1]['VolumeRatio'] > 1.5).sum()
        df.iloc[i, df.columns.get_loc('Volume_Surge_Weeks')] = vol_surge_count
        
        # Identify weekly momentum building pattern
        # Multiple H2s in recent weeks with increasing volume
        if (h2_recent_count >= 2 and vol_surge_count >= 1 and 
            df.iloc[i]['ROC2'] > 3 and df.iloc[i]['ADX'] > 20):
...
# -----------------------------
# Keltner Channel Trend Detection (Weekly)
# -----------------------------
def detect_kc_upper_trend(data):
    """
    Detect high probability trading opportunities using weekly timeframe:
    ...
    weeks_since_breakout = last_bar['Weeks_Since_Breakout']
    ...
    # Recent H2 activity (check last 5 weeks)
    recent_h2_count = last_5_bars['H2_Bar'].sum()
    recent_h2_volume_surges = last_5_bars['H2_Volume_Surge'].sum()
    
    # Weekly pattern indicators
    h2_weeks_recent = last_bar['H2_Weeks_Recent']
    volume_surge_weeks = last_bar['Volume_Surge_Weeks']
    ...
    base_conditions = {
        ...
        'low_congestion': last_bar['Congestion_Weeks'] < 5,  # Low ranging behavior (adjusted for weekly)
        'positive_momentum': last_bar['ROC4'] > 2,  # Positive 4-week momentum
    ...
    h2_conditions = {
        'has_h2_today': has_h2,  # H2 bar this week
        'h2_with_volume': h2_with_volume,  # H2 with volume surge
        'h2_near_kc_break': h2_near_kc,  # H2 near KC breakout
        'multiple_h2_bars': h2_count >= 2,  # Consecutive H2 weeks
        'recent_h2_activity': recent_h2_count >= 2,  # Multiple H2s in last 5 weeks
    }
    ...
    kc_conditions = {
        ...
        'recent_kc_break': weeks_since_breakout <= 3,  # Recent KC breakout
        'strong_kc_distance': last_bar['KC_Distance'] > 1.5,  # Well above KC
        'weeks_above_kc': last_bar['Weeks_Above_KC'] >= 2,  # Sustained above KC
    }
    ...
    advanced_conditions = {
        ...
        'strong_momentum': last_bar['ROC2'] > 5,  # Strong 2-week momentum
    ...
    # Weekly pattern conditions (multi-week momentum building)
    weekly_pattern_conditions = {
        'building_momentum_flag': building_momentum,  # Algorithm detected pattern
        'multi_week_h2': h2_weeks_recent >= 2,  # Multiple H2s in recent weeks
        'volume_confirmation': volume_surge_weeks >= 1,  # At least one volume surge
        'h2_strength_good': h2_strength > 0.5 if has_h2 else False,  # Strong H2 close
        'consistent_momentum': last_bar['ROC2'] > 3 and last_bar['ROC4'] > 5,  # Building momentum
        'ready_to_scale': h2_weeks_recent >= 3 and recent_h2_volume_surges >= 1,  # Time to scale position
    }
    ...
    weekly_score = sum(weekly_pattern_conditions.values())
    
    # Calculate weighted total score
    # Weekly patterns get highest weight as they show sustained momentum
    weighted_score = (
        ...
        weekly_score * 5.0      # Weekly patterns are 5x weighted (highest)
    )
    ...
    max_weekly_score = len(weekly_pattern_conditions)
    max_weighted_score = (max_base_score * 1.0 + max_h2_score * 3.0 + 
                         max_kc_score * 2.0 + max_advanced_score * 4.0 + max_weekly_score * 5.0)
    
    # Require minimum base score of 4 out of 8
    if base_score >= 4:
        # Calculate congestion penalty (lower is better)
        congestion_penalty = last_bar['Congestion_Weeks'] / 10.0  # 0 to 1 scale
        ...
        if weekly_pattern_conditions['ready_to_scale']:
            pattern_type = 'Weekly_Pattern'
            description = 'Multi-week H2 momentum with volume - TIME TO SCALE POSITION!'
        elif weekly_pattern_conditions['building_momentum_flag'] and weekly_pattern_conditions['multi_week_h2']:
            pattern_type = 'Building_Weekly'
            description = 'Building multi-week momentum - WATCH CLOSELY!'
        elif advanced_conditions['h2_kc_combo'] and h2_weeks_recent >= 2:
            pattern_type = 'Strong_H2_KC_Weekly'
            description = 'H2 + KC with multi-week momentum - HIGH PROBABILITY'
        elif has_h2 and h2_with_volume and h2_strength > 1.0:
            pattern_type = 'Power_H2_Weekly'
            description = 'Strong H2 with volume surge - WEEKLY MOMENTUM ACCELERATING'
        elif last_bar['Above_KC_Upper'] and recent_h2_count >= 2:
            pattern_type = 'KC_Multi_H2_Weekly'
            description = 'Above KC with multiple H2s - POSITION BUILDING OPPORTUNITY'
        elif has_h2 and h2_weeks_recent >= 1:
            pattern_type = 'H2_Weekly_Start'
            description = 'H2 pattern starting - POTENTIAL WEEKLY SETUP FORMING'
        elif last_bar['Above_KC_Upper']:
            pattern_type = 'KC_Breakout_Weekly'
            description = 'Above KC upper - MONITOR FOR H2 DEVELOPMENT'
        else:
            pattern_type = 'Early_Weekly_Setup'
        ...
        return {
            ...
            'momentum_4w': last_bar['ROC4'],
            'momentum_2w': last_bar['ROC2'],
...
def process_ticker(ticker):
    """Process a single ticker for Keltner Channel trend patterns on weekly timeframe"""
    logger.info(f"Processing {ticker}")
    
    try:
        now = datetime.datetime.now()
        
        # Extended date range for better pattern recognition (2 years of weekly data)
        ...
        kc_pattern = detect_kc_upper_trend(weekly_with_indicators)
        ...
        logger.info(f"{ticker} - Scores: Base={kc_pattern['base_score']}/{kc_pattern['max_base_score']}, "
                   ...
                   f"Weekly={kc_pattern['weekly_score']}/{kc_pattern['max_weekly_score']}")
        logger.info(f"{ticker} - H2 Status: Has H2={kc_pattern['has_h2']}, H2s recent={kc_pattern['h2_weeks_recent']}, "
                   f"Volume surges={kc_pattern['volume_surge_weeks']}, Building={kc_pattern['building_momentum']}")
        ...
        result = {
            ...
            'Weekly_Score': f"{kc_pattern['weekly_score']}/{kc_pattern['max_weekly_score']}",
            'H2_Weeks_Recent': kc_pattern['h2_weeks_recent'],
            'Volume_Surge_Weeks': kc_pattern['volume_surge_weeks'],
            'Building_Momentum': kc_pattern['building_momentum'],
            'Weeks_Since_KC_Break': kc_pattern['weeks_since_breakout'],
            'Trend_Strength': kc_pattern['trend_strength'],
            'Congestion_Weeks': kc_pattern['congestion_weeks'],
            ...
            'Momentum_4W': kc_pattern['momentum_4w'],
            'Momentum_2W': kc_pattern['momentum_2w'],
...
def generate_html_report(filtered_df, output_file, scanner_file):
    ...
    <head>
        ...
        <title>KC Upper Limit Weekly Filter - {formatted_date}</title>
        <style>
            body {{
    ...
    <body>
        <h1>Keltner Channel Upper Limit Weekly Filter</h1>
        <div class="header-info">
            <div>Date: {formatted_date} | Time: {formatted_time}</div>
            <div>Filtered from: Ticker.xlsx | Weekly Timeframe | Trend Following Strategy</div>
        </div>

        <div class="kc-info">
            <h3>Weekly Strategy Overview</h3>
            <p>This scanner identifies high-probability trading opportunities on weekly timeframe using:</p>
            <ul>
                <li><strong>H2 Pattern:</strong> Two consecutive weeks closing above previous highs (weekly momentum confirmation)</li>
                <li><strong>KC Breakouts:</strong> Price breaking above Keltner Channel upper band on weekly charts</li>
                <li><strong>Volume Surge:</strong> Increased volume on H2 weeks shows institutional accumulation</li>
                <li><strong>Pattern Combos:</strong> H2 + KC + Volume = Highest probability multi-week setups</li>
                <li><strong>Position Building:</strong> Weekly patterns ideal for scaling into positions over time</li>
            </ul>
        </div>

        <h2>High Probability Weekly Setups ({len(filtered_df)} matches)</h2>
    """
    ...
    h2_kc_combos = filtered_df[filtered_df['Pattern'].str.contains('H2_KC', na=False)]
    ...
    if len(h2_kc_combos) > 0:
        html_content += """
        <h3 style="color: #e74c3c;">🔥 Weekly H2 + KC Combos (HIGHEST PROBABILITY)</h3>
        <p>These stocks show both H2 pattern AND KC breakout on weekly timeframe - ideal for multi-week positions!</p>
        """
    elif len(h2_patterns) > 0:
        html_content += """
        <h3 style="color: #f39c12;">📈 Weekly H2 Patterns Detected</h3>
        <p>These stocks show H2 momentum patterns on weekly charts - watch for continuation!</p>
        """
    ...
    for idx, row in filtered_df.iterrows():
        ...
        <div class="ticker-card">
            ...
            <div class="ticker-details">
                ...
                <div class="detail-item">
                    <div class="detail-label">Congestion Weeks</div>
                    <div class="detail-value">{row['Congestion_Weeks']} / 10</div>
                </div>
                ...
                <div class="detail-item">
                    <div class="detail-label">4-Week Momentum</div>
                    <div class="detail-value">{row['Momentum_4W']:.2f}%</div>
                </div>
                
                <div class="detail-item">
                    <div class="detail-label">2-Week Momentum</div>
                    <div class="detail-value">{row['Momentum_2W']:.2f}%</div>
                </div>
                ...
                <div class="detail-item">
                    <div class="detail-label">Recent H2s (5 weeks)</div>
                ...
                <div class="detail-item">
                    <div class="detail-label">Weekly Score</div>
                    <div class="detail-value">{row['Weekly_Score']}</div>
                </div>
            </div>
            ...
            <div class="pattern-info">
                ...
                <strong>Key Insight:</strong> {
                    "Weekly H2 + KC combo detected - HIGHEST probability multi-week setup!" if 'H2_KC' in row['Pattern'] else
                    "H2 with volume surge - Strong weekly momentum building!" if row['Pattern'] == 'Power_H2_Weekly' else
                    "Multiple H2 patterns above KC - Position scaling opportunity!" if row['Pattern'] == 'KC_Multi_H2_Weekly' else
                    "H2 pattern forming - Watch for weekly continuation!" if row['Has_H2'] else
                    "Near KC upper band - Monitor for H2 development on weekly"
                }
                <br>
                <strong>Position Strategy:</strong> {
                    "Scale into position over 2-3 weeks as pattern confirms" if row['Has_H2'] else
                    "Wait for H2 pattern to develop before entry"
                }
                <br>
                <strong>Risk Level:</strong> {"Low" if row['Congestion_Weeks'] < 3 else "Moderate" if row['Congestion_Weeks'] < 5 else "High"} congestion risk ({row['Congestion_Weeks']} weeks).
            </div>
        </div>
        """
    ...
    html_content += f"""
        <div class="source-info">
            <p>Generated on {formatted_date} at {formatted_time} | KC Upper Limit Weekly Filter</p>
            <p><strong>Note:</strong> These setups identify stocks breaking out above Keltner Channel upper band on weekly timeframe.</p>
            <p><strong>Risk Management:</strong> Weekly patterns are ideal for position building. Scale in over multiple weeks.</p>
        </div>
    </body>
    </html>
    """
...
def generate_pdf_report(filtered_df, output_file):
    ...
    try:
        ...
        # Title
        story.append(Paragraph("KC Upper Limit Weekly Filter", title_style))
        story.append(Paragraph(f"Date: {formatted_date} | Time: {formatted_time}", normal_style))
        story.append(Spacer(1, 0.5*inch))
        
        # Strategy Overview
        story.append(Paragraph("Weekly Strategy Overview", heading_style))
        overview_text = """This scanner identifies high-probability trading opportunities on weekly timeframe using:
        • H2 Pattern: Two consecutive weeks closing above previous highs
        • KC Breakouts: Price breaking above Keltner Channel upper band
        • Volume Surge: Increased volume on H2 weeks shows institutional accumulation
        • Pattern Combos: H2 + KC + Volume = Highest probability multi-week setups
        • Position Building: Weekly patterns ideal for scaling into positions"""
        story.append(Paragraph(overview_text.replace('\n', '<br/>'), normal_style))
        story.append(Spacer(1, 0.3*inch))
        
        # Summary
        story.append(Paragraph(f"High Probability Weekly Setups ({len(filtered_df)} matches)", heading_style))
        ...
        if len(filtered_df) > 0:
            ...
            for idx, row in filtered_df.iterrows():
                ...
                metrics_data = [
                    ...
                    ['Volume Ratio', f"{row['Volume_Ratio']:.2f}x", 'Momentum 4W', f"{row['Momentum_4W']:.2f}%"],
                    ['H2 Count', f"{row['H2_Count']}", 'Recent H2s', f"{row['Recent_H2s']}"],
                    ['Base Score', row['Base_Score'], 'H2 Score', row['H2_Score']],
                    ['KC Score', row['KC_Score'], 'Weekly Score', row['Weekly_Score']]
                ]
            ...
            story.append(Paragraph("No KC upper trend patterns found in today's weekly scan.", normal_style))
        ...
        story.append(Paragraph(f"Generated on {formatted_date} at {formatted_time} | KC Upper Limit Weekly Filter", footer_style))
        story.append(Paragraph("Risk Management: Weekly patterns are ideal for position building over multiple weeks.", footer_style))
...
# -----------------------------
def main():
    """Main function to filter tickers for KC upper limit trending patterns on weekly timeframe"""
    logger.info("KC Upper Limit Weekly filter with Sector Information")
    ...
    try:
        ...
        excel_file = os.path.join(RESULTS_DIR, f"KC_Upper_Limit_Weekly_{formatted_date}_{formatted_time}.xlsx")
        html_file = os.path.join(HTML_DIR, f"KC_Upper_Limit_Weekly_{formatted_date}_{formatted_time.replace('_', '-')}.html")
        pdf_file = os.path.join(PDF_DIR, f"KC_Upper_Limit_Weekly_{formatted_date}_{formatted_time}.pdf")
        ...
        if results:
            ...
            numeric_cols = ['Entry_Price', 'Stop_Loss', 'Target1', 'Target2', 'Risk', 'Risk_Reward_Ratio', 
                          'KC_Upper', 'KC_Middle', 'KC_Distance_%', 'ADX', 'Volume_Ratio', 'Momentum_4W', 'Momentum_2W', 
            ...
            priority_cols = ['Ticker', 'Sector', 'Pattern', 'Direction', 'Probability_Score', 
                           ...
                           'Trend_Strength', 'Congestion_Weeks', 
            ...
            try:
                ...
                market_regime_file = os.path.join(market_regime_dir, f"KC_Upper_Limit_Weekly_{formatted_date}_{formatted_time}.xlsx")
            ...
            print("\n===== High Probability Weekly Trading Opportunities =====")
            print(f"Found {len(results_df)} high probability weekly setups")
            
            # Separate by pattern type
            weekly_patterns = results_df[results_df['Pattern'].str.contains('Weekly_Pattern', na=False)]
            h2_kc_combos = results_df[results_df['Pattern'].str.contains('H2_KC', na=False)]
            ...
            if len(weekly_patterns) > 0:
                print(f"\n💎 WEEKLY PATTERNS ({len(weekly_patterns)} stocks) - TIME TO BUILD/SCALE POSITIONS:")
                for idx, row in weekly_patterns.iterrows():
                    action = "SCALE NOW!" if row['Pattern'] == 'Weekly_Pattern' else "WATCH CLOSELY"
                    print(f"  {row['Ticker']} ({row['Sector']}): Score {row['Probability_Score']:.0f}/100, "
                          f"H2s recent: {row['H2_Weeks_Recent']}, Volume surges: {row['Volume_Surge_Weeks']}, "
                          f"Action: {action}, Entry ₹{row['Entry_Price']:.2f}")
            
            if len(h2_kc_combos) > 0:
                print(f"\n🔥 STRONG H2 + KC WEEKLY COMBOS ({len(h2_kc_combos)} stocks) - HIGH PROBABILITY:")
            ...
            if len(h2_patterns) > 0:
                print(f"\n📈 Weekly H2 Patterns Detected ({len(h2_patterns)} stocks):")
            ...
            if len(high_prob) > 0:
                print(f"\n⭐ High Probability Weekly Setups (Score >= 70): {len(high_prob)} stocks")
            ...
            print("\nTop 5 weekly setups by probability score:")
            ...
            empty_cols = ['Ticker', 'Sector', 'Pattern', 'Direction', 'Probability_Score', 
                          'Has_H2', 'H2_Count', 'H2_Volume', 'Recent_H2s',
                          'Base_Score', 'H2_Score', 'KC_Score', 'Advanced_Score', 'Weekly_Score',
                          'H2_Weeks_Recent', 'Volume_Surge_Weeks', 'Building_Momentum',
                          'Weeks_Since_KC_Break', 'Trend_Strength', 'Congestion_Weeks',
                          'Entry_Price', 'Stop_Loss', 'Target1', 'Target2', 'Risk', 'Risk_Reward_Ratio',
                          'KC_Upper', 'KC_Middle', 'KC_Distance_%', 'ADX', 'Volume_Ratio', 
                          'Momentum_4W', 'Momentum_2W', 'Pattern_Strength', 'ATR', 'Description']
            ...
            <body>
                <h1>📈 KC Upper Limit Weekly Filter</h1>
                <div class="no-results">
                    <h2>No KC Upper Weekly Patterns Found</h2>
                    <p>No tickers matched the Keltner Channel upper limit trending criteria on weekly timeframe in today's scan.</p>
                    <p>The filter searched for stocks with:</p>
                    <ul style="display: inline-block; text-align: left;">
                        <li>Price trading above Keltner Channel upper band on weekly chart</li>
                        <li>Strong trending behavior (ADX > 25)</li>
                        <li>Low congestion (less than 5 weeks in narrow range)</li>
                        <li>Positive momentum and volume expansion</li>
                        <li>Score of 6/9 or higher for trend confirmation</li>
                    </ul>
                    <p><strong>Note:</strong> Weekly KC upper patterns are selective and appear during strong multi-week trends.</p>
                </div>
                <div style="margin-top: 50px; color: #999;">
                    <p>Generated on {formatted_date} at {formatted_time.replace('_', ':')} | KC Upper Limit Weekly Filter</p>
                </div>
            </body>
            </html>
            """
            ...
            logger.info(f"No KC upper weekly patterns found. Empty output files created at {excel_file}, {html_file}, and {pdf_file}")
            print("\nNo KC upper weekly patterns found.")
...
if __name__ == "__main__":
    # Print banner
    print("\n===================================")
    print("Weekly High Probability Pattern Scanner")
    print("===================================")
    print("Identifying Weekly Patterns:")
    print("1. H2 Pattern: 2+ weeks closing above previous highs")
    print("2. Multi-week momentum building")
    print("3. Volume surge confirmation for scaling")
    print("4. KC breakout for trend confirmation")
    print("5. Low congestion for clean moves")
    print("")
    print("Position Strategy:")
    print("- Initial: H2 pattern forms on weekly")
    print("- Scale: Volume surge on H2 weeks")
    print("- Hold: For multi-month moves")
    print("===================================")
