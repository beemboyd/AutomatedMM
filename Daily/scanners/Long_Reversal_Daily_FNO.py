#!/usr/bin/env python
# Long_Reversal_Daily_FNO.py - Filter FNO stocks based on higher probability reversal criteria with Sector information:
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                                       "logs", "long_reversal_daily_fno.log")),
...
def parse_args():
    parser = argparse.ArgumentParser(description="Long Reversal Daily FNO Analysis with Sector Information")
...
RESULTS_DIR = os.path.join(os.path.dirname(SCRIPT_DIR), "FNO", "Long", "Liquid")  # Changed output directory
HTML_DIR = os.path.join(os.path.dirname(SCRIPT_DIR), "FNO", "Long", "Liquid")     # Changed HTML directory
...
def generate_html_report(filtered_df, output_file, scanner_file):
    """Generate an HTML report with the filtered FNO stocks"""
    ...
    <head>
        ...
        <title>Long Reversal Daily FNO Filter - {formatted_date}</title>
        <style>
            body {{
    ...
    <body>
        <h1>Long Reversal Daily FNO Filter <span class="fno-badge">F&O</span></h1>
        <div class="fno-notice">
            📊 Futures & Options Segment - High Liquidity Stocks Only
        </div>
        <div class="header-info">
            <div>Date: {formatted_date} | Time: {formatted_time}</div>
            <div>Filtered from: FNO_Liquid.xlsx | LONG ONLY | Higher Probability: 60%+</div>
        </div>

        <h2>FNO Long Reversal Patterns ({len(filtered_df)} matches)</h2>
    """
    ...
    html_content += f"""
        <div class="source-info">
            <p>Generated on {formatted_date} at {formatted_time} | Long Reversal Daily FNO Filter</p>
            <p><strong>Note:</strong> These are F&O stocks with higher probability setups (60%+) and wider stops as per Al Brooks methodology.</p>
        </div>
    </body>
    </html>
    """
...
# -----------------------------
def main():
    """Main function to filter FNO tickers for higher probability LONG reversals with sector information"""
    logger.info("Long Reversal Daily FNO filter with Sector Information")
    ...
    try:
        ...
        excel_file = os.path.join(RESULTS_DIR, f"Long_Reversal_Daily_FNO_{formatted_date}_{formatted_time}.xlsx")
        html_file = os.path.join(HTML_DIR, f"Long_Reversal_Daily_FNO_{formatted_date}_{formatted_time}.html")
        ...
        if results:
            ...
            print("\n===== Long Reversal Daily FNO Results with Sector Information =====")
            print(f"Found {len(results_df)} higher probability LONG reversal patterns in F&O stocks")
            ...
            print("\nTop 5 F&O LONG patterns by score:")
            ...
            <body>
                ...
                <div class="no-results">
                    <h2>No Higher Probability LONG Patterns Found in F&O Stocks</h2>
                    <p>No F&O tickers matched the higher probability LONG reversal criteria in today's scan.</p>
                    <p>The filter searched for BULLISH patterns in F&O stocks with:</p>
                    ...
                    <p><strong>Note:</strong> Higher probability LONG setups in F&O stocks are less frequent but offer better odds when they occur.</p>
                </div>
                <div style="margin-top: 50px; color: #999;">
                    <p>Generated on {formatted_date} at {formatted_time.replace('_', ':')} | Long Reversal Daily FNO Filter</p>
                </div>
            </body>
            </html>
            """
            ...
            logger.info(f"No higher probability LONG patterns found in F&O stocks. Empty output files created at {excel_file} and {html_file}")
            print("\nNo higher probability LONG reversal patterns found in F&O stocks.")
...
if __name__ == "__main__":
    # Print banner
    print("\n===================================")
    print("Long Reversal Daily FNO Filter")
    print("===================================")
    print("Finding higher probability reversal setups (60%+) in F&O stocks:")
