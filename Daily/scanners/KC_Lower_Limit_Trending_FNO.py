#!/usr/bin/env python
# KC_Lower_Limit_Trending_FNO.py - Filter FNO stocks based on Keltner Channel lower limit breakdown with trend following:
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                                       "logs", "kc_lower_limit_trending_fno.log")),
...
def parse_args():
    parser = argparse.ArgumentParser(description="KC Lower Limit Trending Analysis for FNO Stocks with Sector Information")
...
RESULTS_DIR = os.path.join(FNO_DIR, "Short")  # Short trending opportunities
HTML_DIR = os.path.join(FNO_DIR, "Short", "Detailed_Analysis")
PDF_DIR = os.path.join(FNO_DIR, "Short", "PDF")
...
def generate_html_report(filtered_df, output_file, scanner_file):
    ...
    <head>
        ...
        <title>FNO KC Lower Limit Trending Filter - {formatted_date}</title>
        <style>
            body {{
    ...
    <body>
        <h1>FNO Keltner Channel Lower Limit Trending Filter (Shorting) <span class="fno-badge">F&O</span></h1>
        <div class="header-info">
            <div>Date: {formatted_date} | Time: {formatted_time}</div>
            <div>Filtered from: FNO.xlsx | Daily Timeframe | F&O Trend Following Short Strategy</div>
        </div>


        <h2>High Probability F&O Short Setups ({len(filtered_df)} matches)</h2>
    """
    ...
    if len(l2_kc_combos) > 0:
        html_content += """
        <h3 style="color: #e74c3c;">🔥 L2 + KC Combos (HIGHEST PROBABILITY F&O SHORTS)</h3>
        <p>These F&O stocks show both L2 pattern AND KC breakdown - ideal for short futures/put options!</p>
        """
    elif len(l2_patterns) > 0:
        html_content += """
        <h3 style="color: #f39c12;">📉 L2 Patterns Detected in F&O Stocks</h3>
        <p>These F&O stocks show L2 bearish momentum patterns - watch for continuation!</p>
        """
        ...
        <div class="ticker-card">
            ...
            <div class="pattern-info">
                ...
                <strong>F&O Strategy:</strong> {
                    "Sell futures or buy ATM put options - HIGH LEVERAGE OPPORTUNITY" if row['Pattern'] == 'L2_KC_Combo' else
                    "Short futures with strict stop - MOMENTUM ACCELERATING" if row['Pattern'] == 'L2_Volume_Surge' else
                    "Consider bear put spread - RISK DEFINED STRATEGY" if row['Pattern'] == 'KC_Multi_L2' else
                    "Monitor for put entry - BUILDING WEAKNESS" if row['Has_L2'] else
                    "Watch for short setup - EARLY WARNING"
                }
                <br>
                <strong>Position Strategy:</strong> {
                    "Short futures on L2 confirmation, add puts on volume surge" if row['Has_L2'] else
                    "Wait for L2 pattern before initiating shorts"
                }
                <br>
    ...
    html_content += f"""
        <div class="source-info">
            <p>Generated on {formatted_date} at {formatted_time} | FNO KC Lower Limit Trending Filter</p>
            <p><strong>Note:</strong> These F&O setups identify stocks breaking down below Keltner Channel lower band with strong bearish trending characteristics.</p>
            <p><strong>Risk Management:</strong> F&O provides leverage - use appropriate position sizing. Consider put options for defined risk.</p>
        </div>
    </body>
    </html>
    """
...
def generate_pdf_report(filtered_df, output_file):
    ...
    try:
        ...
        # Title
        story.append(Paragraph("FNO KC Lower Limit Trending Filter (Shorting)", title_style))
        story.append(Paragraph(f"Date: {formatted_date} | Time: {formatted_time}", normal_style))
        story.append(Spacer(1, 0.5*inch))
        
        
        # Summary
        story.append(Paragraph(f"High Probability F&O Short Setups ({len(filtered_df)} matches)", heading_style))
        ...
        if len(filtered_df) > 0:
            ...
            story.append(Paragraph("No FNO KC lower trend patterns found in today's scan.", normal_style))
        ...
        story.append(Paragraph(f"Generated on {formatted_date} at {formatted_time} | FNO KC Lower Limit Trending Filter", footer_style))
...
# -----------------------------
def main():
    """Main function to filter FNO tickers for KC lower limit trending patterns"""
    logger.info("FNO KC Lower Limit Trending filter with Sector Information (Shorting)")
    ...
    try:
        ...
        excel_file = os.path.join(RESULTS_DIR, f"KC_Lower_Limit_Trending_FNO_{formatted_date}_{formatted_time}.xlsx")
        html_file = os.path.join(HTML_DIR, f"KC_Lower_Limit_Trending_FNO_{formatted_date}_{formatted_time.replace('_', '-')}.html")
        pdf_file = os.path.join(PDF_DIR, f"KC_Lower_Limit_Trending_FNO_{formatted_date}_{formatted_time}.pdf")
        ...
        if results:
            ...
            print("\n===== High Probability F&O Shorting Opportunities =====")
            print(f"Found {len(results_df)} high probability F&O short setups")
            ...
            if len(bearish_patterns) > 0:
                print(f"\n💎 F&O BEARISH PATTERNS ({len(bearish_patterns)} stocks) - TIME TO BUILD/DOUBLE SHORT POSITIONS:")
            ...
            if len(l2_kc_combos) > 0:
                print(f"\n🔥 F&O STRONG L2 + KC COMBOS ({len(l2_kc_combos)} stocks) - HIGH PROBABILITY SHORTS:")
            ...
            if len(l2_patterns) > 0:
                print(f"\n📉 F&O L2 Patterns Detected ({len(l2_patterns)} stocks):")
            ...
            if len(high_prob) > 0:
                print(f"\n⭐ High Probability F&O Short Setups (Score >= 70): {len(high_prob)} stocks")
            ...
            print("\nTop 5 F&O short setups by probability score:")
            ...
            <body>
                <h1>📉 FNO KC Lower Limit Trending Filter (Shorting)</h1>
                <div class="no-results">
                    <h2>No FNO KC Lower Trend Patterns Found</h2>
                    <p>No F&O tickers matched the Keltner Channel lower limit trending criteria for shorting in today's scan.</p>
                ...
                <div style="margin-top: 50px; color: #999;">
                    <p>Generated on {formatted_date} at {formatted_time.replace('_', ':')} | FNO KC Lower Limit Trending Filter</p>
                </div>
            </body>
            </html>
            """
            ...
            logger.info(f"No FNO KC lower trend patterns found. Empty output files created at {excel_file}, {html_file}, and {pdf_file}")
            print("\nNo FNO KC lower trend patterns found for shorting.")
...
if __name__ == "__main__":
    # Print banner
    print("\n===================================")
    print("F&O High Probability Shorting Scanner")
    print("===================================")
    print("Identifying Bearish G-Pattern setups in F&O stocks:")
    ...
    print("")
    print("F&O Short Position Strategy:")
    print("- Futures: Direct short on L2 pattern")
    print("- Options: Buy ATM/ITM puts")
    print("- Spreads: Bear put spreads for defined risk")
    print("===================================")
