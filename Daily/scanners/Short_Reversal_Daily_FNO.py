#!/usr/bin/env python
# Short_Reversal_Daily_FNO.py - Filter FNO stocks based on higher probability SHORT reversal criteria with Sector information:
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                                       "logs", "short_reversal_daily_fno.log")),
...
def parse_args():
    parser = argparse.ArgumentParser(description="Short Reversal Daily FNO Analysis with Al Brooks L2 Patterns")
...
# -----------------------------
# Higher Probability SHORT Reversal Pattern Detection 
# -----------------------------
def detect_higher_probability_reversal(data):
    """
    Detect higher probability BEARISH reversal patterns based on Al Brooks methodology:
    1. Strong breakdown in new direction (confirmed reversal)
    2. Multiple confirmation bars
    3. Break of significant support
    4. Volume expansion
    5. Trend alignment confirmation

    Returns a dictionary with pattern details if found, None otherwise
    NOTE: This version filters for SHORT setups only
    """
    ...
    # Check for strong bearish reversal pattern ONLY
    bearish_reversal = detect_bearish_confirmation_pattern(data, last_3_bars, last_bar, prev_bar)
    if bearish_reversal:
...
def process_ticker(ticker):
    """Process a single ticker for higher probability SHORT reversal patterns"""
    ...
    try:
        ...
        if reversal_pattern is None:
            logger.info(f"{ticker} - No higher probability SHORT reversal pattern detected")
            return None
        ...
        # Calculate risk and reward (SHORT ONLY)
        ...
        if risk > 0:
            risk_reward_ratio = abs(entry_price - target1) / risk
        else:
            risk_reward_ratio = 0
        ...
        # Log the findings
        logger.info(f"{ticker} - Higher Probability SHORT Reversal Detected!")
...
# -----------------------------
# Read FNO Ticker File
# -----------------------------
def read_ticker_file():
    """Read tickers from the FNO_Liquid.xlsx file"""
    ticker_file = os.path.join(DATA_DIR, "FNO_Liquid.xlsx")
    ...
    try:
        ...
        # Read the Excel file - assuming tickers are in the first column or a column named 'Ticker'
...
def generate_html_report(filtered_df, output_file, scanner_file):
    """Generate an HTML report with the filtered FNO stocks for SHORT"""
    ...
    <head>
        ...
        <title>Short Reversal Daily FNO Filter - {formatted_date}</title>
        <style>
            body {{
    ...
    <body>
        <h1>Short Reversal Daily FNO Filter <span class="fno-badge">F&O</span></h1>
        <div class="fno-notice">
            📉 Futures & Options Segment - High Liquidity SHORT Opportunities
        </div>
        <div class="header-info">
            <div>Date: {formatted_date} | Time: {formatted_time}</div>
            <div>Filtered from: FNO_Liquid.xlsx | SHORT ONLY | Higher Probability: 60%+</div>
        </div>

        <h2>FNO Short Reversal Patterns ({len(filtered_df)} matches)</h2>
    """
    ...
    if len(filtered_df) > 0:
        ...
        <div class="sector-summary">
            <h3>Sector Distribution (F&O Stocks)</h3>
            <ul>
        """
    ...
    for idx, row in filtered_df.iterrows():
        html_content += f"""
            <tr>
                <td>{row['Ticker']} <span class="fno-badge">F&O</span></td>
                <td><span class="sector-badge">{row['Sector']}</span></td>
                <td><span class="ticker-direction">SHORT</span></td>
                <td>{row['Score']}</td>
        ...
        <div class="ticker-card">
            <div class="ticker-header">
                <div class="ticker-name">
                    {row['Ticker']} <span class="fno-badge">F&O</span>
    ...
    html_content += f"""
        <div class="source-info">
            <p>Generated on {formatted_date} at {formatted_time} | Short Reversal Daily FNO Filter</p>
            <p><strong>Note:</strong> These are F&O stocks with higher probability SHORT setups (60%+) and wider stops as per Al Brooks methodology.</p>
        </div>
    </body>
    </html>
    """
...
# -----------------------------
def main():
    """Main function to filter FNO tickers for higher probability SHORT reversals with sector information"""
    logger.info("Short Reversal Daily FNO filter with Sector Information")
    ...
    try:
        ...
        excel_file = os.path.join(RESULTS_DIR, f"Short_Reversal_Daily_FNO_{formatted_date}_{formatted_time}.xlsx")
        html_file = os.path.join(HTML_DIR, f"Short_Reversal_Daily_FNO_{formatted_date}_{formatted_time}.html")
        ...
        if results:
            ...
            # Generate HTML report
            html_output = generate_html_report(results_df, html_file, "FNO_Liquid.xlsx")
            ...
            print("\n===== Short Reversal Daily FNO Results with Sector Information =====")
            print(f"Found {len(results_df)} higher probability SHORT reversal patterns in F&O stocks")
            
            # Print sector summary
            sector_counts = results_df['Sector'].value_counts()
            print("\nSector Distribution (F&O):")
            for sector, count in sector_counts.items():
                print(f"  {sector}: {count} stocks")
            
            print("\nTop 5 F&O SHORT patterns by score:")
            ...
            <body>
                ...
                <div class="no-results">
                    <h2>No Higher Probability SHORT Patterns Found in F&O Stocks</h2>
                    <p>No F&O tickers matched the higher probability SHORT reversal criteria in today's scan.</p>
                    <p>The filter searched for BEARISH patterns in F&O stocks with:</p>
                    <ul style="display: inline-block; text-align: left;">
                        <li>Strong bearish breakdown below support</li>
                        <li>Multiple confirmation bars (at least 2 of last 3 bearish)</li>
                        <li>Break of significant support levels</li>
                        <li>Volume expansion (1.5x+ average)</li>
                        <li>Score of 5/7 or higher for 60%+ probability</li>
                        <li><strong>SHORT SETUPS ONLY</strong> - No long patterns included</li>
                        <li><strong>F&O STOCKS ONLY</strong> - High liquidity names</li>
                    </ul>
                    <p><strong>Note:</strong> Higher probability SHORT setups in F&O stocks are less frequent but offer better odds when they occur.</p>
                </div>
                <div style="margin-top: 50px; color: #999;">
                    <p>Generated on {formatted_date} at {formatted_time.replace('_', ':')} | Short Reversal Daily FNO Filter</p>
                </div>
            </body>
            </html>
            """
            ...
            logger.info(f"No higher probability SHORT patterns found in F&O stocks. Empty output files created at {excel_file} and {html_file}")
            print("\nNo higher probability SHORT reversal patterns found in F&O stocks.")
...
if __name__ == "__main__":
    # Print banner
    print("\n===================================")
    print("Short Reversal Daily FNO Filter")
    print("===================================")
    print("Finding higher probability SHORT reversal setups (60%+) in F&O stocks:")
    print("1. Strong breakdown confirmation in new direction")
    print("2. Multiple confirmation bars")
    print("3. Break of significant support")
    print("4. Volume expansion on breakdown")
    print("5. Wider stops for higher probability")
    print("6. Includes Sector information")
    print("7. F&O STOCKS ONLY - High liquidity names")
    print("===================================")
    ...
    # Don't trigger market regime analysis for FNO scanner
