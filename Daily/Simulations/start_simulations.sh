...
pkill -f "simulation_[1-6]\.py" 2>/dev/null
pkill -f "simulation_dashboard\.py" 2>/dev/null
sleep 2

# Start dashboards
echo "Starting dashboards on ports 4001-4006..."
...
nohup /usr/bin/python3 dashboards/simulation_dashboard.py --sim-id sim_5 --port 4005 > logs/dashboard_4005.log 2>&1 &
nohup /usr/bin/python3 dashboards/simulation_dashboard.py --sim-id sim_6 --port 4006 > logs/dashboard_4006.log 2>&1 &
sleep 3
...
nohup /usr/bin/python3 runners/simulation_5.py > logs/simulation_5.log 2>&1 &
nohup /usr/bin/python3 runners/simulation_6.py > logs/simulation_6.log 2>&1 &
sleep 3
...
sim_count=$(ps aux | grep -E "simulation_[1-6]\.py" | grep -v grep | wc -l)
echo "Simulation runners: $sim_count/6 running"

echo ""
echo "Dashboard URLs:"
echo "  Hourly Timeframe:"
echo "  - http://localhost:4001 (Long Hourly + KC SL)"
echo "  - http://localhost:4002 (Long Hourly + PSAR SL)"
echo "  Short (Intraday):"
echo "  - http://localhost:4003 (Short + KC SL)"
echo "  - http://localhost:4004 (Short + PSAR SL)"
echo "  Daily Timeframe:"
echo "  - http://localhost:4005 (Long Daily + KC SL)"
echo "  - http://localhost:4006 (Long Daily + PSAR SL)"
echo ""
