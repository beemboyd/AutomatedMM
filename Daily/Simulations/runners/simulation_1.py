...
class Simulation1Runner(BaseSimulationRunner):
    """
    Simulation 1: Long + KC Lower SL

    - Long positions with fixed Stop Loss at Keltner Channel Lower band
    ...
    def __init__(self):
        ...
        with open(config_path, 'r') as f:
            config = json.load(f)
        super().__init__('sim_1', config)

        logger.info(f"Simulation 1 initialized: LONG + KC Lower SL")

    def should_enter(self, signal: Dict) -> Tuple[bool, str]:
        """
        Entry logic for Simulation 1 (Long + KC Lower)
        """
        # Base validation
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(log_dir / 'simulation_1.log'),
            logging.StreamHandler()
        ]
    )

    runner = Simulation1Runner()

    import argparse
    parser = argparse.ArgumentParser(description='Simulation 1: Long + KC Lower SL')
    ...
    if args.reset:
        runner.reset()
        print("Simulation 1 reset complete")
    elif args.eod:
        runner.end_of_day()
    elif args.once:
        runner.run_once()
